...
function ConfigFakeMap() {
    ...
    this.renderBaseMap = function() {
        ...
        if (mapInfo.isBA() && sprites.areAllSpritesLoaded()) {
            ...
            mapBaseCanvasCtx.scale(fakeMapBaseCanvasImageData, fakeMapBaseCanvasImageData);
            mapBaseCanvasCtx.drawImage(widthValues, Math.floor(.645 * (currentMapWidth / fakeMapBaseCanvasImageData - widthValues.width)), Math.floor(.422 * currentMapHeight / fakeMapBaseCanvasImageData - widthValues.height / 2));
            mapBaseCanvasCtx.restore();
        }             
        mapLoaded = true;
        mainHandler.canvasDirty = true
