...
function loadMap(mapID, k) {
    ...
    if (mapID !== currentMapID || isnotBAnorRealMap(currentMapID) && k !== currentMapSeed) {
        ...
        if (isRealMap(currentMapID)) configRealMap(firstTime);
        else {
...
function getFPS(firstTime) {
    var secondTime = performance.now();
    frameRate = Math.floor(29 - (secondTime - firstTime) / (33 * mapInfo.getValueByID(currentMapID).per));
...
function a36(g, k, n, l, x) {
    var t, z, y, A = g.length - 1,
        B = (currentMapWidth + currentMapHeight)**2;
    var C = n.length;
    ...
    for (z = currentMapWidth - 1; 0 <= z; z--)
        for (y = currentMapHeight - 1; 0 <= y; y--) {
            ...
            for (t = 1; t < C; t++)
                if (B < n[t]) {
                    D = l[t - 1] + allDivideFloor((B - n[t - 1]) * F[t], E[t]);
                    K = x[t - 1] + allDivideFloor((B - n[t - 1]) * G[t],
                        E[t]);
                    break
...
function configRealMap(firstTime) {
    var mapData;
