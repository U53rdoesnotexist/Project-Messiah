...
function load_password() {
    var password = 0;
    if (is_ios) password = ios.password;
    else if (12 <= device_version) password = droid.loadString(22);
    else {
        if (5 <= device_version) return password;
        password = user_settings.get_settings(9)
    }
...
function cs(g) {
    var k, n = ci[0],
        l = troops[n] + attacks.get_troops_remaining_from_target(n, g);
    for (k = cg - 1; 1 <= k; k--) {
        var x = troops[ci[k]] + attacks.get_troops_remaining_from_target(ci[k], g);
        x < l && (n = ci[k], l = x)
    }
    return n
}
...
function gK() {
    ...
    this.gg = function(J, L, H, M) {
        if (!(canvas_hidden || D && !H && K || 0 === land[J])) {
            gj.gk = !1;
            K = H;
            g(L);
    ...
    this.gp = function(J) {
        ...
        K = !0;
        g(J);
        n(0, 0, map_width - 1, map_height - 1);
        k(.875);
        D = !0;
        gn.go()
    };
...
function hA() {
    function g() {
        ...
        a: {
            for (I = 0; 8 > I; I++)
                if (t = divide_floor(y * ce.random(), ce.value(100)), z = divide_floor(A * ce.random(), ce.value(100)), k()) {
                    I = !0;
                    break a
                } I = !1
        }
        if (!I) a: {
            var D, K, J, L;I = divide_floor(y * ce.random(), ce.value(100));
            var H = divide_floor(A * ce.random(), ce.value(100));
            for (D = 40; 1 <= D; D--)
                for (K = A - D; 0 <= K; K -= 40)
    ...
    this.init = function() {
        ...
        if (dr.ds && dr.dt.hH)
            for (G = 0; G < max_entities; G++) {
                ...
                for (G = 0; G < max_entities; G++) 1 !== is_alive[G] && (G < entity_count && g() ? (D = C + t * B + divide_floor(B, 2), I = E + z * B + divide_floor(B, 2), n(), l(D - 2, I - 2)) : n());
        statistics.numbers[7] = land[my_id];
        statistics.numbers[8] = troops[my_id]
    };
    ...
    this.ho = function(I) {
        G = I;
        if (g()) {
            I = C + t * B + divide_floor(B, 2);
            var D = E + z * B + divide_floor(B, 2);
            n();
            l(I - 2, D - 2)
        } else n()
    }
...
function hp() {
    ...
    eA.cG();
    eK.cG();
    canvas_hidden || (cH.imageSmoothingEnabled = !1, announcements.cG(), eM.cG(), eR.cG(), peace.cG(), eB.cG(), gj.cG(), c2.cG(), eT.cG(), eO.cG(), eP.cG(), fq.cG(), eW.cG(), hu.cG(), hv.cG(), eX.cG())
}
...
function Points_1v1() {
    this.players = null;
    this.init = function(player_info) {
        this.players = player_info;
        announcements.new_1v1(this.players)
    };
    this.calculate_new_elo = function(winner) {
        var elo_change_then_loser_new_elo = 8 / (1 + Math.pow(2, (this.players[winner].elo - this.players[1 - winner].elo) / 10 / 32));
        elo_change_then_loser_new_elo = Math.floor(10 * elo_change_then_loser_new_elo + .5);
        var winner_new_elo = this.round_elo(this.players[winner].elo + elo_change_then_loser_new_elo);
        elo_change_then_loser_new_elo = this.round_elo(this.players[1 - winner].elo - elo_change_then_loser_new_elo);
        0 === winner ? announcements.result_1v1(this.players, winner_new_elo, elo_change_then_loser_new_elo, ["rgba(10,140,10,0.75)", "rgba(140,10,10,0.75)"]) : 
            announcements.result_1v1(this.players, elo_change_then_loser_new_elo, winner_new_elo, ["rgba(140,10,10,0.75)", "rgba(10,140,10,0.75)"])
    };
    this.round_elo = function(elo) {
...
function Names() {
    ...
    this.charcode_to_str = function(x) {
        var t, z = [x.length];
    ...
    this.str_to_charcode = function(x) {
    ...
    this.iX = function(x) {
        ...
        for (z = 0; z < t; z++) {
            ...
            58 > A ? y.push(x[z]) : (A = 91 > A ? A - 65 : A - 71, y.push(String(divide_floor(A, 10))), y.push(String(A - 10 * divide_floor(A, 10))))
        }
        t = y.length - 2;
        A = 0;
        x = [];
...
function Spawn() {
    ...
    this.dF = function() { // in this context meanse xit spawn but others context wrong
        ...
        0 !== is_alive[my_id] ? (statistics.numbers[7] = land[my_id], statistics.numbers[8] = troops[my_id], eR.cE(), eB.j2(), eV.gf(x_min[my_id] - 5, y_min[my_id] - 5, x_max[my_id] + 5, y_max[my_id] + 5), eX.init()) : eW.show(!1, !1);
        announcements.j3(18);
        eA.j4();
        eA.eb();
        spawn = null;
...
function jy() {
    ...
    this.init = function() {
        ...
        B = C = z = y = E = 0;
        this.l3();
        for (var L = this.l4.length - 1; 0 <= L; L--) x.push(!1);
        x.push(!1)
    };
    this.l3 = function() {
        this.l4 = [];
    ...
    this.c7 = function(L, H) {
        ...
        var M = l(L, H);
        if (!n(M) || 0 === M || 6 === M || !x[2] && 3 === M) return this.end(), 2;
        if (1 === M) {
            if (x[6]) {
                M = (new Date).getTime();
                M > J + 4E3 && (K = []);
        ...
        if (4 === M) return x[0] ? in_spawn ? (this.end(), singleplayer ? (spawn.set(0, pixel.to_x(coord), pixel.to_y(coord)), spawn.dF()) : multi.pick_location(1E3, pixel.to_x(coord), pixel.to_y(coord))) : (this.end(), announcements.low_balance(), singleplayer ? single_attack(my_id, F, eR.lV()) : (!free_spawn || 300 < eB.lX()) && multi.attack(eR.lV(), F === max_entities ? my_id : F)) : x[8] ? (this.end(), eH.lZ(F, eR.lV())) : this.end(), 1;
        if (5 === M) return x[1] ? (this.end(), announcements.low_balance(), singleplayer ? single.send_boat(my_id, eR.lV(), pixel.to_x(coord), pixel.to_y(coord)) : multi.pick_location(eR.lV(), pixel.to_x(coord), pixel.to_y(coord)), 1) : 0;
        if (7 === M && x[4]) return this.end(), t = a5.show(L, H), 1;
        if (8 === M) return x[5] ?
            (eQ.lQ(0, [F], !0) && (announcements.la(F, 0), multi.non_aggression(F)), this.end(), 1) : 0;
        this.end();
        return 2
    };
    ...
    this.lo = function(L, H) {
        ...
        return n(l(L, H)) ? !1 : (this.end(), c4.c5 = !0)
    };
    this.end = function() {
        var L;
...
function jz() {
    function g() {
        ...
        B.fillStyle = lz;
        B.fillRect(0, 0, l, l);
        0 === y && (B.fillStyle = m0, B.fillRect(0, 0, l, l));
        B.fillStyle = cK;
        B.fillRect(0, 0, l, 1);
        B.fillRect(0, 0, 1, l);
    ...
    function k(B, C) {
        ...
        if (B <= 4 * l + m7) {
            if (C >= eR.fK) return 0;
            if (C >= eR.fK - l - A * m7) return 2;
            if (C >= eR.fK - 2 * (l + A * m7)) return 3
        } else if (B <= 7 * l + m7 && C >= eR.fK - l - A * m7) return 1;
        return -1
    }

    function n(B, C) {
        cH.setTransform(1, 0, 0, 1, m7, eR.fK - B * A * m7 - B * l);
        cH.fillStyle = lz;
    ...
    this.c7 = function(B, C) {
        var E = k(B, C);
        return this.lw ? 0 === E ? (leave_game(), 2) : 1 === E ? (this.m2(), 2) : 2 === E ? (this.fr(my_id) && (singleplayer ? single.fp(my_id) : multi.surrender(), this.m2()), 2) : 3 === E && 2 <= statistics.m6 ? (hv.m2(), c4.c5 = !0, 2) : hv.hidden ||
    ...
    this.cG = function() {
        if (this.lw) {
            ...
            cH.setTransform(1, 0, 0, 1, m7, eR.fK);
            cH.fillStyle = lz;
            cH.fillRect(0, 0, B, l);
            ...
            B = .4 * l;
            fq.mB(m7 + 4 * l + (1.5 * l - B) / 2, eR.fK + .3 * l, B);
...
function Announcements() {
    ...
    this.c7 = function(H, M) {
        for (var Q = y(), R, P = F.length - 1; 0 <= P; P--)
            if (R = Q - (P + 1) * G, M >= R && M < R + G) {
                if (50 === F[P].id) {
                    if (H >= gE - D - N - F[P].c1) return H >= gE - D - N ? (P = F[P].player, this.la(P, 0), multi.non_aggression(P)) : eV.gg(F[P].player, 800, !1, 0), !0;
                    break
                }
    ...
    this.result_1v1 = function(H, M, Q, R) {
        1 === eD.n2() && (new_announcement(0, H[0].name + ": " + points_1v1.round_elo(H[0].elo) + " -> " + M, 66, 0, cK, R[0], -1, !1), new_announcement(0, H[1].name + ": " + points_1v1.round_elo(H[1].elo) + " -> " + Q, 66, 1, cK, R[1], -1, !1))
    };
...
function ni() {
    ...
    this.nq = 8;
    this.nr = Array(this.nk + this.nl);
    this.ns = Array(this.nk + this.nl);
    this.fA = this.f9 = 0;
    this.zoom = 1;
    this.nt = .2;
    this.lf = 0;
    ...
    this.init = function() {
        ...
        for (g = 0; g < this.nl; g++) this.ny(g,
            this.nk + g, k);
        this.o0();
        this.l7[26] = this.o1(25, 2);
        this.o2()
    };
    this.ny = function(g, k, n) {
        this.a7[k] = !1;
        this.nu[k] = 0;
    ...
    this.o0 = function() {
        ...
        this.l7[this.a6 - 2] = this.o1(this.a6 - 3, 1);
        this.l7[this.a6 - 7] = this.o1(this.a6 - 2, 1);
        this.l7[this.a6 - 6] = this.o1(this.a6 - 7, 1)
    };
    ...
    this.o5 = function() {
        for (var g = this.lf = 0; g < this.a6; g++) this.a7[g] && (this.nu[this.lf++] = g)
    };
    this.lK = function() {
        8 === this.lf && this.nu[0] === this.nn && this.o5()
    };
    this.o6 = function() {
        this.lf = this.nm;
    ...
    this.show = function(g, k) {
        ...
        this.f9 = t + x;
        this.fA = z + l;
        for (x = 0; x < this.lf; x++) this.nr[x] = Math.floor(t + x % n * this.np), this.ns[x] = Math.floor(z + divide_floor(x, n) * this.np);
        return !0
    };
    this.lI = function(g, k) {
        return !(g < this.nr[0] || k < this.ns[0] || g >= this.f9 || k >= this.fA)
    };
    this.lJ = function(g, k, target_id) {
        if (target_id === my_id && this.nv + 190 > c4.time) return !1;
...
var gH = "rgb(0,0,0)",
    ...
    oQ = "rgba(0,40,0,0.8)",
    oR = "rgb(128,255,128)",
    oS = "rgba(255,255,150,0.2)",
    oT = "rgba(0,255,0,0.3)",
    m0 = "rgba(255,255,255,0.3)",
    oU = "rgba(0,40,90,0.75)",
    oV = "rgba(220,0,0,0.6)",
    oW = "rgba(255,100,100,0.8)",
    ...
    oa = "rgb(255,70,70)",
    ob = "rgb(230,0,0)",
    oc = "rgba(0,60,60,0.85)",
    od = "rgb(160,160,255)",
    cK = "rgb(255,255,255)",
    oe = "rgba(255,255,255,0.6)",
    of = "rgba(255,255,255,0.4)",
    nZ = "rgba(255,255,255,0.25)",
    og = "rgba(255,255,255,0.85)",
    oh = "rgba(255,255,255,0.75)",
    oi = "rgb(255,120,100)",
    oj = "rgba(255,255,0,0.5)",
    cI = "middle",
    ok = "bottom",
    cJ = "center",
    mj = "left",
    ...
    bu = "px Arial",
    oo = [bt, om + bt, bt],
    op = "https://play.google.com/store/apps/details?id=territorial.io",
    oq = "https://apps.apple.com/app/id1581110913",
    or = "https://www.youtube.com/watch?v=toZTQ8aRdFc",
    os = "https://discord.gg/pthqvpTXmh",
    ot = "https://www.instagram.com/davidtschacher/",
    nX = "https://territorial.io/cookie_policy",
    ...
    m7, ox, display_buffer_length, oy, oz, p0, p1, cC, 
...
function k2() {
    ...
    function k(C, E) {
        t[C].hx.strokeStyle = t[C].pu ? mD : oZ;
        t[C].hx.fillStyle = cK;
        t[C].hx.fillRect(E - A, 0,
            1, A);
        var F = Math.floor(A / 12);
        t[C].hx.lineWidth = 3 > F ? 3 : F;
        t[C].hx.lineCap = "round";
        E = A + 1;
        t[C].hx.beginPath();
        t[C].hx.moveTo(Math.floor(E - .35 * A), Math.floor(.35 * A));
        t[C].hx.lineTo(Math.floor(E - A + .35 * A), Math.floor(A - .35 * A));
        t[C].hx.stroke();
        t[C].hx.beginPath();
        t[C].hx.moveTo(Math.floor(E - A + .35 * A), Math.floor(.35 * A));
        t[C].hx.lineTo(Math.floor(E - .35 * A), Math.floor(A - .35 * A));
        t[C].hx.stroke()
    }
    ...
    this.c7 = function(C, E) {
        ...
        for (F = t.length -
            ...
            var K = t[F].ls.width;
            if (E >= D - N && E <= D + A + N) {
                if (C >= I - G && C <= I + A + G) return t[F].pu || (t[F].po = !0, t[F].pu = !0, 0 === t[F].id ? singleplayer ? single.cancel(my_id, t[F].cM) : multi.cancel(t[F].cM === max_entities ? my_id : t[F].cM) : singleplayer ? single.cancel_boat(my_id, t[F].id) : multi.cancel_boat(t[F].id)), !0;
                if (0 === t[F].id && C >= I + K - A - G && C <= I + K + G) return singleplayer ? single_attack(my_id, t[F].cM, eR.lV()) : multi.attack(eR.lV(), t[F].cM === max_entities ? my_id : t[F].cM), !0
            }
        }
        return !1
    };
    this.dF = function() {
        if (2 !== client_status && 0 !== is_alive[my_id] && !j8 && hu.lG(my_id)) {
            ...
            b: if (t.length !== C) var E = !0;
                    ...
                    for (E = C - 1; 0 <= E; E--)
                        if (t[E].id !== attacks.get_boat_id_from_attack_index(my_id,
                                E) || t[E].cM !== attacks.get_attack_target_from_attack_index(my_id, E)) {
                            E = !0;
                            break b
                        } E = !1
                }
            if (E) {
                ...
                E = 0;
                b: for (; E < C; E++) {
                    var N = attacks.get_boat_id_from_attack_index(my_id, E);
                    var I = attacks.get_attack_target_from_attack_index(my_id, E);
...
function k3() {
    ...
    function k() {
        ...
        D.lineWidth = G;
        D.fillStyle = 1 === l[0].qR ? og : hy;
        n();
        D.fill();
        D.strokeStyle = 1 === l[0].qR ? gH : cK;
        n();
        D.stroke();
        D.textAlign = cJ;
        D.textBaseline = cI;
        D.fillStyle = 1 === l[0].qR ? gH : cK;
        D.font = A[0];
        D.fillText(E[l[0].qQ], Math.floor(l[0].width / 2), Math.floor(.72 * C[0] * y));
        D.font = A[1];
        D.fillText(l[0].l,
...
function k4() {
    ...
    this.lo = function(I, D) {
        return this.pV ? l(I, D) : !1
    };
    this.pY = function() {
        this.pV = !1
    };
...
function k6() {
    ...
    this.lo = function(P, U) {
        U > cB - .6 * y ? this.ri ? P !== K && (z += P - K, K = P, k(), n(P), this.ri = -1 !== G, c4.c5 = !0) : n(P) && (c4.c5 = !0) : this.pW()
    };
    this.pW = function() {
...
function k8() {
    function g() {
        var S;
        B.clearRect(0, 0, sI, qU);
        B.fillStyle = oU;
        B.fillRect(0, 0, sI, N);
        B.fillStyle = oH;
        B.fillRect(0, N, sI, qU - N);
        sM[my_id] >= V && n(sM[my_id] - V, oT);
        0 !== sM[my_id] && 0 === V && n(0, oS); - 1 !== X && n(X, m0);
        B.fillStyle = oJ;
        B.fillRect(0, N, sI, 1);
        B.fillRect(0, 0, sI, ox);
        B.fillRect(0, 0, ox, qU);
        B.fillRect(sI - ox, 0, ox, qU);
        B.fillRect(0, qU - ox, sI, ox);
        B.font = sJ;
        B.textBaseline = cI;
        B.textAlign = cJ;
...
function tu() {
    ...
    this.tx = function() {
        ...
        if (this.h9 || this.tv && this.j5) {
            var g = gy.ty[0],
                k = gy.ty[1],
                n = gy.ty[2],
                l = gy.ty[3];
            g = g < this.tw[0] ? this.tw[0] : g;
            k = k < this.tw[1] ? this.tw[1] : k;
...
function ks() {
    ...
    this.u7 = function(g, k) {
        announcements.low_balance();
        singleplayer ? single_attack(g, k, eR.lV()) : multi.attack(eR.lV(), k === max_entities ? g : k)
    };
    this.u6 = function(g) {
        g.sort(function(k, n) {
...
function u9() {
    ...
    this.bv = function() {
        c4.time - 12E4 >= n[this.uA] && (n[this.uA] = c4.time, this.uP = [0, 0], eD.rk(0, 1 + this.uA) && multi.load_leaderboard(0, this.uA))
    };
    this.lx = function() {
        var l;
    ...
    this.uS = function(l, x, t, z, y, A, B) {
        this.position[l] = B;
        var C = x.length,
    ...
    this.c7 = function(l, x) {
        ...
        n[this.uA] = c4.time;
        eD.rk(0, 1 + this.uA) && multi.load_leaderboard(0, this.uA);
        return !0
    };
    this.cG = function() {
        if (this.hidden) {
...
function kB() {
    ...
    this.rs = function() {
        ...
        this.uz[0].fK = Math.floor(.54 * cB);
        this.uz[1].fK = this.uz[0].fK;
        this.uz[2].fK = Math.floor((cB - this.uz[2].cw - this.uz[3].cw - this.f6) / 2);
        this.uz[3].fK = this.uz[2].fK + this.uz[2].cw + this.f6;
        this.uz[4].fK = Math.floor((cB -
...
function v9() {
    ...
    this.c7 = function(k, n) {
        ...
        k -= 2 * this.f6 + this.vB;
        if (n < this.f6 + this.vD) return this.vA = 1, this.colors[this.nu][0] = g(256 * k / this.vC), c4.c5 = !0;
        if (n < 2 * this.f6 + this.vD) return !0;
        if (n < 2 * this.f6 + 2 * this.vD) return this.vA = 2, this.colors[this.nu][1] = g(256 * k / this.vC), c4.c5 = !0;
        n >= 3 * this.f6 + 2 * this.vD && (this.vA = 3, this.colors[this.nu][2] = g(256 * k / this.vC), c4.c5 = !0);
        return !0
    };
    this.vG = function() {
...
function kC() {
    ...
    function k() {
        D++;
        I = c4.time;
        eD.rk(g(), 4) && (J = !0, multi.join_lobby(g()))
    }

    function n() {
        0 === D ? jj.vd(3249) : (D === eD.vX - 1 && (D = -1), k())
    }
    ...
    this.rm = function(L) {
        6 !== aJ.pa() || J || (I = c4.time, J = !0, multi.join_lobby(L))
    };
    this.c7 = function(L, H) {
    ...
    this.dF = function() {
        ...
        F = "rgba(0," + Math.floor(1.9 * x) + "," + Math.floor(1.2 * x) + "," + (.8 - .004 * x) + ")", c4.c5 = !0)
    };
    this.cG = function() {
...
function kD() {
    ...
    this.vr = function() {
        return nW.end() || cC.us() ? !0 : cD.hidden ? (cD.hidden = !1, !0) : !1
    };
    this.aK =
        function() {
            c4.c5 = !0;
    ...
    this.cG = function() {
        if (8 !== g && 10 !== g) {
            ...
            cC.cG();
            jt.cG();
            0 === g ? je.cG() : 2 === g ? dy.cG() : 3 === g ? jj.cG() : 5 === g ? jl.cG() : 6 === g ? ji.cG() : 7 === g && lobby.cG();
            cC.vy();
            cookies_window.cG();
            cD.cG();
            nW.cG()
...
function w1() {
    ...
    this.c7 = function(g, k, n) {
        ...
        if (0 ===
            n || 1 === n && !a5.a7[g] || 2 === n && a5.a7[g]) this.w2 = !a5.a7[g], this.w3 = this.vA = !0, a5.a7[g] = !a5.a7[g], a5.o5(), c4.c5 = !0;
        return !0
    };
    this.lo = function(g, k) {
...
function kE() {
    function g() {
        var t = aJ.pa();
        0 === t ? je.tj() : 6 === t ? eD.vf(n) : 7 === t ? (lobby.tj(), eD.close(eD.w6, 3240)) : 8 === t && (leave_game(), je.tj());
        aJ.setState(3);
        jh.rs();
        jh.uz[2].na = k(n);
        c4.c5 = !0
...
function ki() {
    ...
    this.eZ = function() {
        0 < l && c4.time > n && (l--, n += 4500, 0 === c4.wO && 0 === c4.wP && (0 === l && eD.jc < eD.wQ && (eD.jc += eD.wR), eD.rk(eD.jc, 5)))
    };
    this.wS = function() {
        10 === aJ.pa() &&
            (k++, 2 <= k && (jp.wT(g), g = null))
    }
}
            ...
            for (var P = -1, U = next_games.length - 1; 0 <= U; U--)
                ...
                if (null !== U) next_games[P].ls = U;
                    ...
                    var W = map_height,
                        ...
                        ca = current_map,
                        pa = x7;
                    xJ(next_games[P].map_id, next_games[P].xB);
                    xK.xL();
                    ...
                    O.setTransform(T, 0, 0, T, (128 - T * map_width) / 2, (128 - T * map_height) / 2);
                    O.drawImage(hs, 0, 0);
                    map_width = U;
                    map_height = W;
                    hs = X;
...
function kH() {
    ...
    this.xZ = function() {
        eD.jc = 0;
        eD.rk(0, 3) && multi.single_loaded(0);
        aJ.vp();
        if (dr.ds) dr.xb();
        else {
    ...
    this.c7 = function(n, l) {
        ...
        } else if (this.dz[t].mv === z) return this.dz[t].mv = 1, !0;
        this.dz[t].mv += y;
        1 > this.dz[t].mv ? this.dz[t].mv = 1 : this.dz[t].mv > z && (this.dz[t].mv = z);
        return !0
    };
    this.xX = function() {
...
function kI() {
    function vote() {
        if (0 !== username.indexOf("vote ")) return !1;
        var split_names = username.split(" ");
        if (2 !== split_names.length) return !1;
        je.vote_id = split_names[1];
        display_username();
        eD.rk(0, 7) && multi.vote(0);
        jj.vd(3252);
        return !0
    }
    ...
    function save_account() {
        if (0 !== username.indexOf("account ")) return !1;
        var split_names = username.split(" ");
        if (2 !== split_names.length) return display_username(), jj.vd(3266), !0;
        var password_max_limit = Math.floor(Math.pow(2, 48));
    ...
    function l() {
        if (void 0 !== username && names.iN(username)) return jk.wE(0, !0), !0;
        jk.wE(0, !1);
        return !1
    }
    var username;
    ...
    this.wD = function() {
        ...
        if ("password" === username || "account" === username) {
            var t = names.charcode_to_str(load_password().toString());
            username = "account " + t;
            jk.bz(0).input.value = username
        }
    };
...
function User_settings() {
    ...
    function l() {
        for (var index = const_10 - const_4 - 1; 0 <= index; index--) settings_array[index] = names.str_to_charcode(settings_array[index]);
        settings_array[0] = names.iW(settings_array[0])
    }
    ...
    this.format_settings = function() {
        if (2 === A) {
            ...
            settings_array[0] = names.iV(settings_array[0]);
            for (C = const_10 - const_4 - 1; 0 <= C; C--) settings_array[C] = names.charcode_to_str(settings_array[C]);
...
function kk() {
    function g(l) {
        for (k -= 2; l < k; l += 2) n[l] = n[l + 2], n[l + 1] = n[l + 3]
    }
    var k = 0,
            ...
            if (0 === is_alive[my_id] || attacks.sent_boats_count(my_id) === attacks.ongoing_attack_count(my_id)) k = 0;
                ...
                for (l = k - 2; 0 <= l; l -= 2) {
                    ...
                    if (x < max_entities && 0 === is_alive[x]) g(l);
                        ...
                        var t = n[l + 1];
                        if (x >= max_entities && lh(my_id) || x < max_entities && lm(my_id, x)) singleplayer ? single_attack(my_id, x, t) : multi.attack(t, x === max_entities ? my_id : x), g(l)
                    }
                }
            }
    };
    this.lZ = function(l, x) {
        var t;
        a: {
            for (t = 0; t < k; t += 2)
                if (n[t] === l) {
...
function kr() {
    ...
    function k(z) {
        ...
        A.onload = function() {
            map_width = A.width;
            map_height = A.height;
            4096 < map_width || 4096 < map_height || 10 > map_width || 10 > map_height ? (xJ(0, 0), alert("Image w & h must be between 10 and 4096.")) : (current_map = ze, x7 = 0, hs.width = map_width, hs.height = map_height, pl.drawImage(A, 0, 0), y = pl.getImageData(0, 0, map_width, map_height), xI = y.data)
        };
        A.src = z;
        dr.dt.zz = "";
        return !0
    ...
    this.xb = function() {
        this.dt.ya &&
            this.dt.zj && (this.dt.ya[0] = cC.mE[2].ih.vH());
        game_init(this.dt.zk, 0, this.zl(), this.dt.jJ, !1)
    };
    this.zl = function() {
        return [{
    ...
    this.zo = function(z) {
        ...
        this.dt.zr = l(z.modeID, 0, 1);
        this.dt.x6 = l(z.mapID, 0, ze - 1);
        this.dt.xB = l(z.seedMap, 0, 16383);
        this.dt.zk = l(z.seedSpawn, 0, 16383);
        ...
        y.zn = A;
        this.dt.zz = "string" === typeof z.mapBase64 ? z.mapBase64 : "";
        this.dt.zm = this.dt.zm || !this.dt.zn;
        this.dt.jJ = 0 === this.dt.zr ? 7 : 2 === this.dt.zr ? 9 : 6;
        this.dt.hH = this.dt.hS ? this.dt.hH : null
    };
    this.lJ = function() {
        k(this.dt.zz) || xJ(this.dt.x6, this.dt.xB)
    };
    this.a06 = function() {
        var z, y = 0,
            A = this.dt.xY;
        for (z = 0; z < A; z++) this.dt.zw[z] >
            y && (y = this.dt.zw[z]);
...
function Attacks() {
    ...
    this.get_boat_id_from_attack_index = function(id, attack_index) {
        return boat_ids[starting_index_in_attack_array(id) + attack_index]
    };
    ...
    this.get_attack_troops_remaining_from_attack_index = function(id, attack_index) {
        return remaining_troops[starting_index_in_attack_array(id) + attack_index]
    };
...
function kO() {
    ...
    this.init = function() {
        ...
        if (8 === gamemode) {
            ...
            for (V.font = bt + Math.floor(100 * L) + bu, T = 80 / Math.floor(V.measureText(eP.split_into_pieces(troop_cap)).width), V.font = bt + 100 + bu, O = max_entities - 1; 0 <= O; O--) D[O] = 100 / Math.floor(V.measureText(nickname[O]).width), I[O] = T < D[O] ? T : D[O];
    ...
    this.eb = function() {
        return 0 >= --na ? (na = 4 >= so ? 10 : 12 > so ? 5 : 2, k(), !0) : !1
    };
    this.gG = function(O) {
        return I[O]
    };
...
function kp() {
    ...
    this.lQ = function(g, k, n) {
        return this.a1e(this.a1a, g, k, n)
    };
    this.a1f = function(g, k, n) {
        return this.a1e(this.a1b, g, k, n)
    };
    this.a1e = function(g, k, n, l) {
        var x;
        a: {
            var t;
...
function kj() {
    ...
    this.init = function() {
        ...
        var x = ce.a1W(0);
        ce.jN(0);
        for (l = 1; l < this.wQ; l++) this.a1l[l] = names.iZ() + ".com";
        ce.jN(x);
        ws_array = Array(this.vX);
        n = Array(this.vX);
        for (l = this.vX - 1; 0 <= l; l--) n[l] = {
            a1p: !1,
            time: 0,
            a1q: !1
        }
    ...
    this.dF = function() {
        for (var remote = this.vX - 1; 0 <= remote; remote--) this.vT(remote) && c4.time > n[remote].time + 15E3 && multi.anti_disconnect_15s(remote, n[remote].a1q)
    };
    this.rk = function(l, x) {
        if (!n[l].a1p) return this.a1t(l, x), !1;
        if (ws_array[l].a1u()) return ws_array[l].a1v(x), ws_array[l].vT();
        ws_array[l].end();
        this.a1t(l, x);
        return !1
    };
    this.a1t = function(remote, x) {
        n[remote].a1p = !0;
        n[remote].time = c4.time;
        n[remote].a1q = !1;
        ws_array[remote] = new Terri_ws;
        ws_array[remote].init(remote, x)
    };
    ...
    this.send = function(remote, data) {
        n[remote].time = c4.time;
        n[remote].a1q = !1;
        ws_array[remote].send(data)
    };
    this.pM = function(l) {
        n[l].a1q = !0
    };
    this.close = function(remote, reason) {
        g(remote) && ws_array[remote].close(reason)
    };
    this.a22 = function(l, x) {
        jj.vd(x);
        g(l) && ws_array[l].close(x)
    };
    this.vf = function(l) {
        for (var x = this.vX - 1; 0 <= x; x--) this.close(x, l)
    };
    this.a23 = function(l, x) {
        for (var t = this.vX - 1; 0 <= t; t--) t !== l && this.close(t, x)
    };
...
function main() {
    ...
    window.addEventListener("error", on_error, !0);
    so = 10;
    xK = new a2P;
    gy = new a2Q;
    h8 = new tu;
    ...
    hv = new a2S;
    cookies_window = new Cookies_window;
    c4 = new a2T;
    c4.jd();
    c4.init();
    teams = new Teams;
...
function on_error(g) {
    has_had_error_before || (has_had_error_before = !0, g.message ? (a2F = g.lineno, a2G = g.message, eD.rk(0, 6) && multi.a20(0), g = "[A_ERROR " + a2F + "][" + a2G + "]", a2W(g)) : (g = "[B_ERROR " + g.type + "][" + (g.srcElement || g.target) + "]", console.log(g)))
}
...
function on_key_down(g) {
    "ArrowLeft" === g.key ? gn.wE(3) : "ArrowUp" === g.key ? gn.wE(0) : "ArrowRight" === g.key ? gn.wE(1) : "ArrowDown" === g.key ? gn.wE(2) : "a" === g.key ? eR.r5(.96875) : "d" === g.key ? eR.r5(32 / 31) : "s" === g.key ? eR.r5(.875) : "w" === g.key ? eR.r5(8 / 7) : "1" === g.key ? eR.r5(5 / 6) : "2" === g.key ? eR.r5(1.2) : " " === g.key && jw.u0()
}

function on_visibility_change() {
    "hidden" === document.visibilityState ? c4.a2X() : c4.xt()
}

function on_key_up(g) {
    400 > c4.time || (8 !== aJ.pa() && aJ.us(g) ? c4.c5 = !0 : "Escape" === g.key ? aJ.aK() : "ArrowLeft" === g.key ? gn.a2Y(3) : "ArrowUp" === g.key ? gn.a2Y(0) : "ArrowRight" === g.key ? gn.a2Y(1) : "ArrowDown" === g.key ? gn.a2Y(2) : "h" === g.key && 1 <= client_status && (canvas_hidden = !canvas_hidden, c4.c5 = !0))
}

function a2K() {
    a29 = window.location.hostname;
    a2C = 0 <= a29.toLowerCase().indexOf("territorial.io")
}
...
var map_width, map_height, hs, pl, xH, xI, current_map, x7, vq, ze = 14,
    xK;

function xJ(g, k) {
    g %= ze;
    if (g !== current_map || a2h(current_map) && k !== x7) {
...
function a2n(g) {
    ...
    so = Math.floor(29 - (k - g) / (33 * jm.bz(current_map).per));
    so = 0 > so ? 0 : 20 < so ? 20 : so
}
...
function a2P() {
    ...
    this.dF = function() {
        if (8 === aJ.pa() && eV.gd()) this.rf = setTimeout(g, 16);
            ...
            if (0 === this.aA) {
                var l = ce.a1W();
                ce.jN(jm.bz(current_map).a33[2]);
                jn.jT([map_width, map_height, jm.bz(current_map).a33[0], jm.bz(current_map).a33[1]]);
                ce.jN(l);
                this.a2y = jn.a35();
                this.aA++;
                if (this.a2z) {
            ...
            l = this.a2z ? 10 : 1E6;
            l =
                map_height - this.a2s - 1 < l ? map_height - this.a2s - 1 : l;
            l = this.a2s + l;
            for (var x, t, z = this.a2s; z < l; z++)
                for (var y = 1; y < map_width - 1; y++)
                    if (t = y + z * map_width, x = 4 * t, n(x)) this.a37(x, t, 1);
                    else {
                        this.a37(x, t, 0);
                        t = y;
                        var A = z;
                        (1 < t && n(x - 4) || t < map_width - 2 && n(x + 4) || 1 < A && n(x - 4 * map_width) || A < map_height - 2 && n(x + 4 * map_width)) && this.a38(y, z)
                    } this.a2s = l;
            this.a2s >= map_height - 1 ? (pl.putImageData(xH, 0, 0, 1, 1, map_width - 2, map_height - 2), c4.c5 = !0, this.xL()) : this.a2z && (this.rf = setTimeout(g, 16))
        }
    };
    ...
    this.a38 = function(l, x) {
        var t = l - this.a2u;
        var z = x - this.a2u;
        var y = l + this.a2u,
            A = x + this.a2u;
        t = 1 > t ? 1 : t;
        y = y > map_width - 2 ? map_width - 2 : y;
        A = A > map_height - 2 ? map_height - 2 : A;
        for (var B = 1 > z ? 1 : z; B <= A; B++)
            for (var C = t; C <= y; C++)
                if (z = 4 * (C + B * map_width), n(z)) {
...
function a2k() {
    2 === current_map ? a3E([256], [256], [0, 205, 256], [500, 500, 0], [0, 0, 0]) : 7 === current_map ? a3E([512], [512], [0, 380, 512], [500, 500, 0], [0, 0, 0]) : 8 === current_map ? a3E([410], [410], [0, 120, 210], [0, 80, 640], [0, 0, 0]) : 9 === current_map && a3E([512], [512], [0, 70, 180, 200, 290, 420, 512], [500, 500, 0, 0, 500, 500, 0], [0, 0, 0, 0, 0, 0, 0])
}

function a3E(g, k, n, l, x) {
    var t, z, y, A = g.length - 1,
        B = map_width + map_height;
    B *= B;
    var C = n.length;
    ...
    for (z = map_width - 1; 0 <= z; z--)
        for (y = map_height - 1; 0 <= y; y--) {
            var I = B;
            for (t = A; 0 <= t; t--) {
            ...
            for (t = 1; t < C; t++)
                if (I < n[t]) {
                    D = l[t - 1] + yj((I - n[t - 1]) * F[t], E[t]);
                    K = x[t - 1] + yj((I - n[t - 1]) * G[t],
                        E[t]);
                    break
                } a3S(map_width * y + z, D, K, N)
        }
}

function a3S(g, k, n, l) {
    500 > k ? l[g] = divide_floor(l[g] * k * 2, 1E3) : 500 < k && (l[g] += divide_floor(2 * (1E4 - l[g]) * (k - 500), 1E3));
    l[g] += divide_floor(n * (10 * k - l[g]), 1E3)
}
...
function kR() {
    ...
    this.a2l = function() {
        ...
        for (z = map_width * map_height - 1; 0 <= z; z--)
            for (y = B; 0 <= y; y--)
                if (A[z] >= n[y]) {
                    var N = A[z] -
                        n[y];
                    xI[4 * z] = l[y] + yj(E[y] * N, C[y]);
                    xI[4 * z + 1] = x[y] + yj(F[y] * N, C[y]);
                    xI[4 * z + 2] = t[y] + yj(G[y] * N, C[y]);
                    xI[4 * z + 3] = 255;
                    break
    ...
    this.jR = function() {
        ...
        l = 4 * map_width;
        for (k = map_height - 1; 0 <= k; k--) x[k * l + 2] = 3, x[k * l + l - 2] = 3;
        for (k = map_width * map_height - 1; 0 <= k; k--) l = 4 * k, 3 !== x[l + 2] && (t[l + 2] > t[l + 1] && t[l + 2] > t[l] ? x[l + 2] = 2 : (x[l + 2] = 1, n++));
        this.nG = (map_width - 2) * (map_height - 2);
        this.nL = 0;
        if (a2q(current_map)) {
            t = 0;
            l = pixel_rgba;
...
function a2j(g) {
    var k;
    current_map === a2p() ? k = "" : current_map ===
        ...
        current_map === a2p() + 2 ? k = "" :
        current_map === a2p() + 3 && (k = "");
    k = names.iX(k);
...
function kS() {
    ...
    this.init = function() {
        ...
        g[10] = {
            ...
            a31: [140, 130, 120],
            a32: [12, 12, 76],
            a33: [240, 120, 4672, 30, 26, 30, 90, 8, 32, 3, 9],
            per: 1
        };
        g[11] = {
            name: "World",
            a31: [165, 145, 125],
            a32: [15, 15, 69],
            a33: [250, 100, 8, 25, 15, 25, 90, 8, 32, 3, 9],
            per: 1
        };
        g[12] = {
            name: "Caucasia",
            a31: [140, 130, 120],
            a32: [20, 20, 84],
            a33: [240, 120, 100, 30, 25, 30, 90, 8, 32, 3, 9],
            per: 1
        };
        g[13] = {
            name: "USA 48",
            ...
            a32: [12, 12, 60],
            a33: [300, 300, 9827, 26, 18, 36, 36, 8, 32, 3, 9],
            per: 1
        }
    };
    this.a3W = function() {
...
function kT() {
    ...
    function n(J, L) {
        var H = J - C[L - 1];
        if (0 !== H) {
            var M = 1 + divide_floor(Math.abs(H), L - 1);
            M = 0 > H ? -M : M;
            C[L - 1] = J;
            var Q = L - 1 - divide_floor(Math.abs(H), Math.abs(M));
            Q = 1 > Q ? 1 : Q > L - 2 ? L - 2 : Q;
    ...
    this.jT = function(J) {
        ...
        for (J = 0; J < t; J++)
            if (!N[J])
                for (L = 0; L < z; L++) I[L] || (H = x[L * t + J - 1] + x[(L - 1) * t + J], M = 2, N[J + 1] && (M++, H += x[L * t + J + 1]), I[L + 1] && (M++, H += x[(L + 1) * t + J]), x[L * t + J] = divide_floor(H, M))
    };
    this.a35 = function() {
        return x
    };
...
function divide_floor(g, k) {
    return Math.floor(g / k + 1 / (2 * k))
}

function yj(g, k) {
    return 0 <= g ? divide_floor(g, k) : -divide_floor(-g, k)
}
...
function t7(g, k, n) {
    return k < g ? g : k > n ? n : k
}

function a4K(g, k) {
    for (var n = divide_floor(g + 1, 2), l = 0; l < k; l++) n = divide_floor(n + divide_floor(g, n), 2);
    return n
}

function bl(g, k) {
    return 1 > g ? 0 : a4K(g, k)
}

function a4L(g, k, n, l, x, t, z, y) {
    return !(g + n <= x || k + l <= t || g >= x + z || k >= t + y)
}

function a4M(g, k, n, l, x, t, z, y) {
    return g <= x && k <= t && g + n >= x + z && k + l >= t + y
}

function kl() {
    function g() {
        vv.uu[2] = vv.uu[3] = vv.uu[4] = !jt.a4N;
        var y = jt.a1U ? 1 : 0,
            A = jt.a4N ? 1 : 0,
            B = jt.a0p ? 1 : 0;
    ...
    function k(y, A, B, C) {
        if (0 === C) return y >= B.f7 && (0 === C || A >= B.f8) && A <= B.f8 + B.nR;
        A -= C * (B.nR - 2);
        return y >= B.f9 && (0 === C || A >= B.f8) && A <= B.f8 + B.nR
    }
    ...
    this.init = function() {
        ...
        if (y) k = null;
            ...
            for (y = 3; 0 <= y; y--) {
                ...
                var C = 0 === y % 2 ? map_width : g;
                var E = 0 === y % 2 ? g : map_height + 2 * g;
                k[y].width = C;
                k[y].height = E;
            ...
            pl.fillStyle = "rgb(" + B + "," + B + "," + B + ")";
            pl.fillRect(0, 0, map_width, 1);
            pl.fillRect(0, map_height - 1, map_width, 1);
            pl.fillRect(0, 0, 1, map_height);
            pl.fillRect(map_width - 1, 0, 1, map_height)
        }
    };
    this.hr = function() {
        var x = n ? 0 : -g;
...
function Terri_ws() {
    ...
    function ws_onmessage(F) {
        A++;
        1 === A ? data_reader.readAsArrayBuffer(F.data) : B.push(F.data)
    }

    function l() {
        A--;
        jp.a5W(remote, new Uint8Array(data_reader.result));
        0 < A && (data_reader.readAsArrayBuffer(B[0]), B.shift())
    }

    function ws_onerror() {}

    function ws_onclose(F) {
        eD.a24(remote, F)
    }
    var remote, y, A, B, data_reader, ws;
    this.init = function(remote_param, G) {
        remote = remote_param;
        y = G;
        A = 0;
        B = [];
    ...
    this.end = function() {
        ...
        ws.onclose = pass;
        ws.onerror = pass;
        data_reader.removeEventListener("loadend", l)
    }
}

function Statistics() {
    this.cV = 501;
    this.a5X = new Uint32Array(this.cV);
    this.s8 = new Uint32Array(this.cV);
    this.tV = new Uint16Array(this.cV);
    this.m6 = 0;
    this.a5Y = 1;
    this.dV = 0;
    this.max = [0, 0, 0];
    this.numbers = 0;
    this.description = "Avg. Attack Strength;Number Attacks;Ships sent;Bots conquered;Humans conquered;Attacked by Bots;Attacked by Humans;Territorial Loss;Territorial Income;Interest Income;Received Support;Overall Income;Commanding Costs;Attack Losses;Defense Losses;Shipping Losses;Transmitted Support;Overall Expenses".split(";");
    this.init = function() {
        this.m6 = 0;
        this.a5Y = 1;
        this.dV = 0;
    ...
    this.a5e = function() {
        this.a5a();
        this.a5d(0);
        this.m6 = 1 + divide_floor(this.cV, 2);
        for (var g = 1; g < this.m6; g++) this.a5X[g] = this.a5X[2 * g], this.s8[g] = this.s8[2 * g], this.tV[g] = this.tV[2 * g], this.a5d(g);
        this.a5Y *=
            2
    };
    this.a5a = function() {
    ...
    this.a5d = function(g) {
        this.max[0] = this.a5X[g] > this.max[0] ? this.a5X[g] : this.max[0];
        this.max[1] = this.s8[g] > this.max[1] ? this.s8[g] : this.max[1];
        this.max[2] = this.tV[g] > this.max[2] ? this.tV[g] : this.max[2]
    }
}
...
function a2S() {
    ...
    this.show = function() {
        2 > statistics.m6 || (this.hidden = !0, this.a5j())
    };
    this.end = function() {
        this.hidden = !1;
        this.a5h = -1
    ...
    this.a5o = function(g, k) {
        ...
        cH.fillText(eP.split_into_pieces(statistics.numbers[9]), 0, 1 * l /
            9);
...
function Teams() {
    ...
    this.a69 = function(l, x, t) {
        ...
        var A = divide_floor(player_count, team_count);
...
function li(g, k) {
    var n, l = attacks.ongoing_attack_count(g);
    for (n = 0; n < l; n++)
        if (0 === attacks.get_boat_id_from_attack_index(g, n)) {
            var x = attacks.get_attack_target_from_attack_index(g, n);
            if (x === max_entities) {
...
function a2T() {
    ...
    function k() {
        var n = performance.now();
        c4.time + 1500 < n && (c4.time = n, c4.a6V())
    }
    this.c5 = !1;
...
function a6e() {
    ...
    this.a6m = function() {
        return 0 < this.a6h.length ? (this.wO++, jp.a6j(this.a6h[0], g), g = (g +
            1) % 8, this.a6h.shift(), !0) : !1
    }
}
...
function km() {
    ...
    this.a6o = function() {
        ...
        this.a6n ? 55 > n || -1 !== c4.a6Y || (this.a6n = !1, n = k.getUTCMinutes(), 4 === n % 5 ? (k = k.getUTCHours(), 59 === n && 15 <= k && 21 >= k ? g("Upcoming Game of the Day", 0) : 14 === n % 30 ? g("Upcoming Alliance Contest", 0) : 29 === n % 30 ? g("Upcoming Battle Royale Contest", 0) : g("Upcoming One-vs-One Game", 8)) : 2 === n % 5 && g("Upcoming Zombie Game",
            9)) : 55 > n && (this.a6n = !0)
    }
}
...
function kZ() {
    ...
    var n, l, x, t;
    this.pP = function(z) {
        return 1 < z.touches.length ? (k(z), hu.end(), !0) : !1
    };
    this.pU = function(z) {
        ...
        if (1 < z.touches.length) {
            ...
            k(z);
            z = g();
            gj.rL(Math.floor((n + x) / 2), Math.floor((l + t) / 2), z / y);
            return c4.c5 = !0
        }
        return !1
    }
...
function ka() {
    ...
    function k(x, t) {
        for (var z = 0, y, A, B = l; B < l + t; B++) y = divide_floor(B, 8), A = 7 - B % 8, z |= (x[y] >> A & 1) << l + t - B - 1;
        l += t;
        return z
    }
    var n, l;
    this.a5W = function(x, t) {
        l = 0;
        n = t.length;
        if (0 === n) eD.a22(x, 3205);
        else {
            var z = k(t, 1);
            if (0 === z)
                if (z = k(t, 2), 0 === z)
                    if (0 === k(t, 1)) {
                        ...
                        if (8 !== aJ.pa())
                            if (3 > n) eD.a22(x,
                                ...
                                0 === y ? cD.uS(0, C, 10, 1, .36, .55, A) : cD.uS(1, C, 100, 2, .47, .5, A)
                            }
                    }
            else if (1 === z)
                if (x !== eD.w6) eD.close(x, 3239);
                else if (6 === aJ.pa() && lobby.init(), 7 !== aJ.pa()) eD.close(x, 3251);
            else {
                y = [0, 0, 0, 0];
                A = k(t, 6);
    ...
    this.wT = function(x) {
        ...
        if (2 ===
            ...
            C = k(x, 14);
            E = [];
            for (D = 0; 2 > D; D++) F = k(x, 1), G = [k(x, 6), k(x, 6), k(x, 6)], elo = k(x, 14), nick = g(k(x, 5), x), E.push({
                name: nick,
                xd: G,
                elo: elo,
                status: F
    ...
    this.wM = function(x) {
        ...
        var t = k(x,
            ...
            z = k(x, 10);
        eD.w6 > eD.wR && (z += eD.wR);
        if (eD.w6 === z) return eD.jc = z, !1;
        eD.close(eD.w6, 3247);
        eD.jc = z;
        eY.wG = k(x, 10);
        eY.wH = k(x, 2 === t ? 9 : 1);
        eD.rk(z, 5) && multi.multi_loaded();
        return !0
    };
    this.a6j = function(x, t) {
        l = 2;
        var z = 8 * x.length;
        if (k(x, 3) !== t) eD.a22(eD.jc, 3248);
        else
            for (; l + 8 <= z;) {
                var y = k(x, 3);
                var A = k(x, 9);
                if (0 === y) {
                    ...
                    var C = k(x, 11);
                    single.fb(A, y, B, C)
                } else 2 === y ? (B = k(x, 9), B = B === A ? max_entities : B, single.fe(A, B)) : 3 === y ? single.fh(A) : 4 === y ? (y = k(x, 7), eA.n4(A, 0, y)) : 5 ===
                    y ? single.fp(A) : 6 === y ? single.fg(A, k(x, 1)) : 7 === y && single.ff(A, 1 + k(x, 11))
            }
    }
}
...
function a2Q() {
    ...
    this.gz = function() {
        ...
        this.ty[3] = Math.floor(this.ty[1] + this.a5K + 1);
        h8.tv = !0
    }
}
...
function kg() {
    ...
    this.c7 = function(g, k) {
        ...
        xJ(n, Math.floor(16384 * Math.random()));
        return !0
    };
    this.cG = function() {
...
function Multi() {
    function g(t) {
        var password = load_password(),
            y = Math.floor(password / 16777216);
        l(t, 24, y);
        l(t, 24, password - 16777216 * y)
    }
    ...
    function n(t) {
        return divide_floor(t, 8) + (0 < t % 8 ? 1 : 0)
    }

    function l(t, z, y) {
        for (var A, B, C = x; C < x + z; C++) A = divide_floor(C, 8), B = 7 - C % 8, t[A] |= (y >> z - (C - x + 1) & 1) << B;
        x += z
    }
    var x;
    this.ws_ready = function() {
    ...
    this.join_lobby = function(lobby) {
        var name_keycodes = names.iT(je.xc()),
            name_length = name_keycodes.length,
            A = new Uint8Array(n(105 + 10 * name_length));
        x = 0;
        l(A, 1, 0);
        l(A, 3, 1);
        l(A, 10, time_hash);
        ...
        g(A);
        k(A);
        for (B = 0; B < name_length; B++) l(A, 10, name_keycodes[B]);
        eD.w6 = lobby;
        eD.send(lobby, A)
    };
    this.load_leaderboard = function(remote_id, board_type) {
        ...
        l(y, 14, version_hash);
        l(y, 1, board_type);
        l(y, 16, Math.abs(4096 + cD.position[board_type] + cD.uP[board_type]) % 65536);
        eD.send(remote_id, y)
    };
    this.a20 = function(t) {
    ...
    this.vote = function(remote_id) {
        var z;
        if (!(je.xq + 7E3 > c4.time)) {
            je.xq = c4.time;
            ...
            l(y, 3, 2);
            g(y);
            var A = me(je.vote_id.length - 20, 0);
            for (z = je.vote_id.length - 1; z >= A; z--) l(y, 4, Math.abs(je.vote_id.charCodeAt(z) - 48) % 10);
            eD.send(remote_id, y)
        }
    };
    this.anti_disconnect_15s = function(remote_id, z) {
    ...
    this.attack = function(ratio, target) {
    ...
    this.pick_location = function(ratio, x_coord, y_coord) {
    ...
    this.cancel_boat = function(boat_id) {
        var z = new Uint8Array(2);
        x = 0;
        l(z, 1, 1);
        l(z, 3, 2);
    ...
    this.upload_1v1_result = function(troop_hash, result) {
    ...
    this.self_emoji = function(emoji) {
        var z = new Uint8Array(2);
        x = 0;
        l(z, 1, 1);
        l(z, 3, 5);
    ...
    this.non_aggression = function(friend) {
        var z = new Uint8Array(2);
        x = 0;
        l(z, 1, 1);
        l(z, 3, 6);
    ...
    this.request_attack = function(friends, target) {
        var friend_index, friends_length = friends.length, 
        B = new Uint8Array(n(15 + 9 * friends_length));
        x = 0;
        l(B, 1, 1);
        l(B, 3, 6);
        l(B, 2, 2);
        l(B, 9, target);
        for (friend_index = 0; friend_index < friends_length; friend_index++) l(B, 9, friends[friend_index]);
        eD.send(eD.jc, B)
    };
    this.vote_peace = function(choice) {
        var z = new Uint8Array(1);
        x = 0;
        l(z, 1, 1);
        l(z, 3, 7);
