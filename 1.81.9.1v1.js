try {
	...
	function tickincrement() {
		...
		if (!isalive[myid] || opponent != null && !isalive[opponent.id]) return 0

		for (let atk of myattacks) {
			if (atk.target == 512 && cycle <= (singleplayer ? 7 : 6) && (tick + latency) % 100 == 5) cancel(512)
		}
	...
	function attack(amount, target) {
		...
		else if (ratio > 700 && !(cycle >= 6 && cycle <= 9)) ratio = 700
