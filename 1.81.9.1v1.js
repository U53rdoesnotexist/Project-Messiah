var tick, cycle, latency, rating, opponentid, pixel_list, old_myattacks;
...
function gameinit() {
	tick = 0, cycle = 1, old_myattacks = [], pixel_list = [];
...
function tickincrement() {
	...
	console.log(tick, aq.t3(myid), myattacks, old_myattacks);

	if (tick + latency == 71 && cycle == 1) multi.attack(Math.ceil(228/troops[myid] * 1000 / (((Math.floor((tick + latency)/10)) - Math.floor(tick/10) >= 1) ? (1 + (aq.t3(myid) - latency * 0.3)/1E4) : 1)), myid)
	if (tick >= 100) {
		tick -= 100;
		cycle += 1;
