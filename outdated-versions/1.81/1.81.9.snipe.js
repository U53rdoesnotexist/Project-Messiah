var tick, cycle, latency, rating, allies, targets, myindex, mytarget, lastlocation;
...
function gameinit() {
	...
	//console.clear();
	console.log(`ID: ${myid}, Players: ${playercount}`);

	targets = [], allies = [];
	for (let id = 0; id < playercount; id++) {
		if (nickname[id].toLowerCase().includes('[' + tag + ']')) targets.push(id) //Fix by checking immediately behind the first mention of [
		else if (nickname[id].toLowerCase().includes('еz')) allies.push(id)
	}
	myindex = allies.findIndex(id => id == myid);
	if (myindex >= 4 * targets.length) location.reload()
	mytarget = targets[Math.floor(myindex / 4)];
	lastlocation = [0,0];
}
...
function tickincrement() {
	...
	if (!isalive[mytarget] || !isalive[myid] || density(mytarget) >= 150) location.reload()
	if (distance(centroid(mytarget)[0] - centroid(myid)[0], centroid(mytarget)[1] - centroid(myid)[1]) >= 30) (console.log(x_min[myid], x_max[myid], lastlocation), location.reload())

	if (cycle == 1 && tick == 20 || cycle >= 2 && tick == 65) multi.attack(500, myid)
	if (tick % 20 == 0 && density(myid) >= 1.5) multi.attack(800, mytarget)
	if (cycle >= 11) location.reload()
}

function check_spawn() {
	if (spawning_percentage_left >= 0.95) {
		if (distance(centroid(mytarget)[0] - lastlocation[0], centroid(mytarget)[1] - lastlocation[1]) >= 15) {
			let offsetx = [7, 0, -3, 0], offsety = [0, 7, 0, -3], pos = myindex % 4;
			lastlocation = [x_min[mytarget] + offsetx[pos], y_min[mytarget]+ offsety[pos]];
			multi.chooselocation(1E3, lastlocation[0], lastlocation[1]);
		}
	}
}
...
function next_game() {
	if (lobbygames == undefined) {
		setTimeout(function () {next_game();}, 1000);
		return 0
	} else {
		for (var game of lobbygames) {
			if (game.lobbygamemode <= 6) {
				multi.joingame(game.gameID);
				return 1
			}
		}
		setTimeout(function () {next_game();}, 3000);
		return 0
	} 
}
...
function centroid(id) {
	return [(x_min[id] + x_max[id])/2, (y_min[id] + y_max[id])/2]
}
...
function kO() {
	...
	function x(P, U) {
		...
		for (X = 0; X < y[1]; X++) {
			var ba = A;
			for (W = 0; W < y[0]; W++) {
				if (P > ba && P < ba + n && U > pa && U < pa + n) return multi.joingame(lobbygames[V].gameID), K = lobbygames[V].gameID !== K ? lobbygames[V].gameID : -1, bw.bx = !0;
				V++;
...
function k4() {
	...
	function t() {
		z[0] = "[еz] ban " + ['paper','omega'][Math.round(Math.random())];
		...
		z[9] = "0"; // ulk: 463736; zor: 514043, add 1-10 behind
		f.y()
	}
