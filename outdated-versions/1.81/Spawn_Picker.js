...
var mapwidth, mapheight, lobbygames, spawning_percentage_left, bot_timing, myattacks;

var pixel_list, spawns, info;

//function game() {

function gameinit() {
	console.clear();
	console.log("Instructions: Choose the most desirable and an alternative spawn for the map. If possible, the alternative spawn will have a long distance to the best one. " + 
	"Afterwards, you can click E to save the file. You can click as many spawns as you want; the last 2 will be saved as your choices. You can remove unwanted spawns by pressing R. " +
	"After that, an image and a text file will occcur and the game will reload to generate a new map. You can continue picking spawns and when you are finished simply close the client window to finish. " + 
	"Finally, please put all the image files in one folder named img and all text files in one folder named txt, then zip the documents and send it to Teinc3 for processing. Thank you very much.")

	pixel_list = [], spawns = [];
	for (let coord = 0; coord < mapheight*mapwidth; coord++) {
		for (let i = 0; i <= 3; i++) {
			pixel_list.push(pixel.isneutral(coord*4) ? 0 : 255)
		}
	}
...
function savespawn() {
	spawns = spawns.slice(spawns.length - 2);
	
	var canvas = document.createElement('canvas'),
	ctx = canvas.getContext('2d');
	canvas.width = mapwidth;
	canvas.height = mapheight;
	var idata = ctx.createImageData(mapwidth, mapheight);
	idata.data.set(pixel_list);
	ctx.putImageData(idata, 0, 0);

	var a = document.createElement("a"),
		url = canvas.toDataURL();
        a.href = url;
        a.download = currentmap.toString()+'_map';
	...
	for (let i = 0; i < 2; i++) {
		download(`(${pixel.pixeltox(spawns[i])},${pixel.pixeltoy(spawns[i])})`, currentmap.toString() +"_spawn_" + i.toString(), 'text');
	}
	pixel_list = [], spawns = [];
}
	...
	this.d7 = function () {
		...
		for (y = 0; y < A; y++)
			if (0 === t[y]) singleattack(k[y], x[y], l[y]);
				...
				var B = k[y];
				0 !== isalive[B] && 2 !== fH[k] && eG.fL(B, 1 === x[y])
...
function jh() {
	...
	this.bz = function (N, G) {
		...
		if (4 === M) return x[0] ? in_spawn ? (this.kx(), singleplayer ? this.nextspawn(H) : multi.chooselocation(1E3, pixel.pixeltox(H), pixel.pixeltoy(H))) : (this.kx(), newannouncements.lA(), singleplayer ? singleattack(myid, E, eF.lB()) : (!freespawn || 300 < dz.lD()) && multi.attack(eF.lB(), E === maxentities ? myid : E)) : x[8] ? (this.kx(), e5.lF(E, eF.lB())) : this.kx(), 1;
	...
	this.nextspawn = function(pixelcoords) {
		var x = pixel.pixeltox(pixelcoords), y = pixel.pixeltoy(pixelcoords);
		newspawn.cI(0, x, y);
		spawns.push(pixelcoords);
		console.log([x,y], spawns.length);
		if (spawns.length >= 6) {
			console.log('Spawns Reset');
			spawns = [];
		}
	}
	...
	this.bz = function (G, M) {
		for (var Q = y(), S, P = E.length - 1; 0 <= P; P--)
			if (S = Q - (P + 1) *
				H, M >= S && M < S + H) {
				if (50 === E[P].id) {
					if (G >= g2 - D - K - E[P].bt) return G >= g2 - D - K ? (P = E[P].player, this.lG(P, 0), multi.lH(P)) : eJ.gU(E[P].player, 800, !1, 0), !0;
					break
				}
...
function kQ() {
	...
	this.bz = function (D, L) {
		...
		I = D > r - lo - this.bt /
			2;
		singleplayer ? this.fL(0, I) : he.kw(myid) && 0 !== isalive[myid] && multi.qE(I);
		return !0
	};
	this.d7 = function () {
		if (0 < J) J--, 0 === J && k();
...
function keyDown(g) {
	if (g.key === 'e' && spawns.length >= 2) (savespawn(), spawns = [], systemgameinit(Math.round(Math.random()*16384), 0, info, 7, false))
	else if (g.key === 'r' && spawns.length >= 1) {
		let lastspawn = spawns.pop();
		console.log(`Spawn No. ${spawns.length + 1} (${pixel.pixeltox(lastspawn)}, ${pixel.pixeltoy(lastspawn)}) removed from list`);
		newspawn.cI(myid, pixel.pixeltox(spawns[spawns.length - 1]), pixel.pixeltoy(spawns[spawns.length - 1]));
	}
