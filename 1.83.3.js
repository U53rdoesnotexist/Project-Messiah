...
function kB() {
    ...
    function k() {
        D++;
        I = c4.time;
        wsManager.ri(g(), 4) && (J = !0, dataEncoder.joinLobby(g()))
    }
        ...
        for (z = aZ * aa - 1; 0 <= z; z--)
            for (y = B; 0 <= y; y--)
                if (A[z] >= n[y]) {
                    var N = A[z] -
                        n[y];
                    x7[4 * z] = l[y] + allDivideFloor(E[y] * N, C[y]);
                    x7[4 * z + 1] = x[y] + allDivideFloor(F[y] * N, C[y]);
                    x7[4 * z + 2] = t[y] + allDivideFloor(G[y] * N, C[y]);
                    x7[4 * z + 3] = 255;
                    break
