...
var mapwidth, mapheight, lobbygames, spawning_percentage_left, accountinfo, lobbygamejoined;
var bot_timing, myattacks;

var ui = true, link = "discord.gg/FaUH5efj";
var allies, targets, mytargets, sent;
...
function gameinit() {
    ...
    allies = [], targets = [], sent = false;
    for (let id = 0; id < playercount; id++) {
        if (nickname[id].includes(link)) {
            allies.push(id)
        } else targets.push(id)
    }
    myindex = allies.findIndex(id => id == myid);
	mytargets = targets.slice(myindex*3,myindex*3+3)
    if (mytargets.length == 0) {
        rejoin_lobby();
        return 0
    }
    console.clear();
	console.log(`ID: ${myid}, Allies: ${allies.length}, Targets: ${targets.length}, My Targets: ${mytargets}`);
...
function rejoin_lobby() {
    leavegame();
    setTimeout(function () {jO.vF()}, 500);
}

function next_game() {
	if (lobbygames == undefined || lobbygames.length == 0) {
		setTimeout(function () {next_game();}, 500);
		return 0
	} else {
		if (lobbygamejoined == -1) {
            multi.joingame(lobbygames[0].gameID);
            lobbygamejoined = lobbygames[0].gameID;
        }
		setTimeout(function () {next_game();}, 4000);
		return 0
	} 
}
...
function advertise() {
    if (!sent) {
        sent = true;
        for (let id of mytargets) {
            multi.message_emojis(100,id);
        }
        setTimeout(function () {rejoin_lobby()}, 500);
    }
}
...
function aT(g) {
	...
	aU();
	aV();
	for (g = attacks.currentattackcount(last_action_owner) - 1; 0 <= g; g--) 0 === attacks.boat_id(last_action_owner, g) && (aD = g, aZ());
	aP && aa()
}
...
function border_changetoattacking(g, k) {
	for (var t = temp_borderpixels[k].length - 1; t >= g; t--) pixel.changetoattacking(temp_borderpixels[k][t], k)
}
...
function d3(id, attackamount) {
	teamgame && (cz[id] = 0);
	if (attacks.belowattackcap(id) && !(60 > attackamount))
	...
	if (0 !== dO.dP[g] && !(5 === t && troops[g] < l || 4 === t && troops[g] < strange_divide_floor(l, 2)))
		for (t = cW.cX(alivecount), l = 0; l < alivecount; l++) {
			...
			if (dO.dP[x] === dO.dP[g] && 1 === cz[x]) {
				dS(g, x, k);
				x < playercount && cW.random() < cW.value(10) && (cz[x] = 0);
				break
			}
		}
}
...
function dW() {
	...
	this.bh = function () {
		...
		send_ratio = new Uint16Array(botcount);
		...
		if (9 === gamemode) this.dj();
		else if (singleplayer)
			if (teamgame)
				for (z = botcount - 1; 0 <= z; z--) this.cF[z] = dl.dm[dO.dP[z + playercount] - 1].b5;
			else
		...
		for (z = botcount - 1; 0 <= z; z--) {
			2 >= this.cF[z] ? (l[z] = 5, x[z] = n[z] = 1040, 0 === this.cF[z] ? (send_ratio[z] = 1E3, t[z] = 1E3) : 1 === this.cF[z] ? (send_ratio[z] = 1E3, t[z] = 920, x[z] = n[z] = 1100) : (send_ratio[z] = 1E3, t[z] = 870)) : 4 >= this.cF[z] ? (l[z] = 1 + cW.cX(20), n[z] = 250 + cW.cX(1501), x[z] = 500 + cW.cX(501), 3 === this.cF[z] ? (send_ratio[z] = 600 + cW.cX(101), t[z] = 300 + cW.cX(401)) : (send_ratio[z] = 300 + cW.cX(201), t[z] = 100 + cW.cX(201))) : (x[z] = 1E3, n[z] = 1E3, l[z] = 35 + cW.cX(16), send_ratio[z] = 400 + cW.cX(101), t[z] = 50 + cW.cX(101)), bot_timing[z] = 1 + strange_divide_floor(x[z] * cW.random(),
				10 * cW.value(100))
		}
	...
	this.d7 = function (z) {
		if (0 === --bot_timing[z]) {
			...
			send_ratio[z] !== t[z] && (send_ratio[z] += send_ratio[z] < t[z] ? l[z] : -l[z], send_ratio[z] = Math.abs(send_ratio[z] - t[z]) <= l[z] ? t[z] : send_ratio[z]);
			bot_timing[z] = strange_divide_floor(x[z], 10);
			var y = z + playercount;
			d3(y, strange_divide_floor(send_ratio[z] * troops[y], 1E3))
		}
	}
}
...
function eR() {
	function g(A) {
		var B;
		for (B = l - 1; 0 <= B; B--) 0 === n[x[B]] && land[x[B]] >= A && aT(x[B])
	}

	function k(A) {
		n[A] = 10 === n[A] ? t : 1E3 > land[A] ? 3 : 1E4 > land[A] ? 2 : 6E4 > land[A] ? 1 : 0
	}
	var t, l, x, n, z, y;
	this.bh = function () {
		z = y = 0;
...
function f0() {
	...
	this.bh = function () {
		...
		for (var t = botcount - 1; 0 <= t; t--) k[t] = t
	};
	this.d7 = function () {
		for (var t = g - 1; 0 <= t; t--)
			if (0 === isalive[k[t] + playercount]) {
				var l = t;
...
function oldsingle() {
	...
	this.d7 = function () {
		...
		for (y = 0; y < A; y++)
			if (0 === t[y]) singleattack(k[y], x[y], l[y]);
				...
				var B = k[y];
				0 !== isalive[B] && 2 !== fH[k] && newpeace.singlevotepeace(B, 1 === x[y])
	...
	this.fd = function (y) {
		0 !== isalive[y] && 2 !== fH[y] && fe.ff(y) && (1 === alivecount ? fW.fX(y) : (newannouncements.fZ(y, y === myid ? 21 : 22), 8 === gamemode ? fW.fX(1 - y) : singleplayer ? (fa(y), fb(), in_spawn && newspawn.exit()) : this.fY(y)))
	}
}
...
function hZ() {
	...
	dy.c8();
	e8.c8();
	canvashidden || (c9.imageSmoothingEnabled = !1, newannouncements.c8(), eA.c8(), eF.c8(), newpeace.c8(), dz.c8(), gX.c8(), bu.c8(), eH.c8(), eC.c8(), eD.c8(), fe.c8(), eK.c8(), he.c8(), hf.c8(), eL.c8())
}
...
function old1v1points() {
	this.players = null;
	this.setplayers = function (g) {
		this.players = g;
		newannouncements.start_1v1(this.players)
	};
	this.calculate_elo = function (g) {
		var elo_change = 8 / (1 + Math.pow(2, (this.players[g].elo - this.players[1 - g].elo) / 10 / 32));
		elo_change = Math.floor(10 * elo_change + .5);
		var t = this.display_elo(this.players[g].elo + elo_change);
		elo_change = this.display_elo(this.players[1 - g].elo - elo_change);
		0 === g ? newannouncements.result_1v1(this.players, t, elo_change, ["rgba(10,140,10,0.75)", "rgba(140,10,10,0.75)"]) : newannouncements.result_1v1(this.players, elo_change, t, ["rgba(140,10,10,0.75)", "rgba(10,140,10,0.75)"])
	};
	this.display_elo = function (g) {
...
function systemgameinit(g, my_id, playerinfo, game_mode, is_contest) {
	...
	newspawn = (freespawn = in_spawn = teamgame || 100 > playercount) ? new oldspawn : null;
	startingtroops = 512;
...
var d8, am, d6, e7, single, e8, eJ, il, m, he, fe, newannouncements, jP, eD, bu, eF, gX, jQ, eC, eA, dz, eK, jR, jS, jT, jU, jV, jW, dl, jO, bo, pixel, f, attacks, aq, dy, jC, dq, jA, jX, jY, gb, en, cW, fp, ha, jZ, ja, eL, multi, jb, eB, lobby, jd, newpeace, eM, e1, e5, je, jf, e6, e3, eE;
...
function jh() {
	...
	this.bz = function (N, G) {
		...
		//4: Attack/Spawn 5: Boat 7: Emoji 8: Non Aggression
...
function ji() {
	...
	this.bz = function (B, C) {
		var F = k(B, C);
		return this.ld ? 0 === F ? (leavegame(), showad(), 2) : 1 === F ? (this.lj(), 2) : 2 === F ? (this.ff(myid) && (singleplayer ? single.fd(myid) : multi.surrender(), this.lj()), 2) : 3 === F && 2 <= newstatistics.ln ? (hf.lj(), bw.bx = !0, 2) :
...
function oldannouncements() {
	...
	function y() {
		return eF.mJ(newannouncements.mG()) ? newpeace.canvotepeace ? eF.f8 - eF.co - 2 * K : eF.f8 - K : newpeace.canvotepeace ? s - eF.co - (q ? 3 : 2) * K : s - K
	}
	...
	function F(G, M) {
		var Q,
			S = !1;
		for (Q = E.length - 1; 0 <= Q; Q--) E[Q].id !== G || M !== maxentities && E[Q].player !== M || (E.splice(Q, 1), S = !0);
		return S
	}
	var E, H, K, J, D, L, I, N;
	this.bh = function () {
		...
		var G = "Map: " + jX.br(currentmap).name + "   Pixels: " + eD.splitpieces(jA.mu) + "   Land: " + eD.splitpieces(jA.mv) + " (" + dz.mw(100 * jA.mv / jA.mu, 1) + ")";
	...
	this.mG = function () {
		if (newpeace.canvotepeace) return newpeace.bt;
		var G = E.length;
	...
	this.bz = function (G, M) {
		for (var Q = y(), S, P = E.length - 1; 0 <= P; P--)
			if (S = Q - (P + 1) *
				H, M >= S && M < S + H) {
				if (50 === E[P].id) {
					if (G >= g2 - D - K - E[P].bt) return G >= g2 - D - K ? (P = E[P].player, this.lG(P, 0), multi.non_aggression(P)) : eJ.gU(E[P].player, 800, !1, 0), !0;
					break
				}
	...
	this.iZ = function (G) {
		bu.cI(G, 2);
		100 > playercount ? A(0, nickname[G] + " won the game.", 3, G, cC, hi, -1, !0) : A(0, nickname[G] + " has been immortalized!", 3, G, cC, hi, -1, !0);
		eJ.gU(G, 2700, !0, 0)
	};
	this.display_emoji = function (G, M, Q) {
		G === myid ? A(175, " Message to " + nickname[M] + ": ", 1E3 + Q, M, C(200, 255, 210), hi, -1, !0) : this.display_received_emoji(G, Q)
	};
	this.display_received_emoji = function (G, M) {
		var Q, S = 0;
		A(175, nickname[G] + ": ", 1E3 + M, G, cC, "rgba(5,60,25,0.9)", -1, !0);
	...
	this.start_1v1 = function (G) {
		A(300, G[0].name + " [" + new1v1points.display_elo(G[0].elo) + "] vs " + G[1].name + " [" + new1v1points.display_elo(G[1].elo) + "]", 65, 0, g5, "rgba(100,255,255,0.75)", -1, !1)
	};
	this.mg = function (G) {
		A(200, G, 0, 0, "rgb(40,255,200)", "rgba(10,60,40,0.9)", -1, !1)
	};
	this.result_1v1 = function (G, M, Q, S) {
		1 === e1.mi && (A(0, G[0].name + ": " + new1v1points.display_elo(G[0].elo) + " -> " + M, 66, 0, cC, S[0], -1, !1), A(0, G[1].name + ": " + new1v1points.display_elo(G[1].elo) + " -> " + Q, 66, 1, cC, S[1], -1, !1))
	};
	this.iX = function (G) {
		1 === e1.mi && A(0,
	...
	this.l7 = function (G, M) {
		...
		a: {
			for (S = G.length - 1; 0 <= S; S--)
				if (2 * land[G[S]] > land[myid]) {
					S = !1;
					break a
				} S = !0
		}
		S ? (Q += "ordered ", S = C(255, 235, 210)) : (Q += "asked ", S = C(210, 255, 210));
		1 < G.length ? A(230, Q + G.length + " players to attack " + nickname[M] + ".", 66, M, S, hi, -1, !0) : A(230, Q + nickname[G[0]] + " to attack " + nickname[M] + ".", 66, G[0], S, hi, M, !0)
	};
	this.mn = function (G, M) {
		land[G] > 2 * land[myid] ? A(230, nickname[G] + " orders you to attack " + nickname[M] + "!", 66, G, cC, "rgba(90,50,5,0.9)", M, !0) : A(230, nickname[G] + " asks you to attack " + nickname[M] + ".", 66, G, cC,
	...
	this.lA = function () {
		100 <= troops[myid] || A(80, "Your balance is too low!", 9, 0, cC, hi, -1, !1)
	};
	this.l9 = function () {
	...
	this.n0 = function (G) {
		...
		var Q = g[G];
		l[G] = 0;
		1 === M ? (M = nickname[Q] + z[G], 0 === G && (M += nickname[k[G]] + "."), A(t[G], M, 7, k[G], cC, hi, -1, !0)) : 2 <= M && (M = nickname[Q] + " and " + (M - 1) + " other player" + (2 === M ? "" : "s") + n[G], A(t[G], M, 7, Q, cC, hi, -1, !1))
	};
	...
	this.d7 = function () {
		...
		if (128 !== I && (I++, !(128 > I)))
			for (G = 5, M = alivecount - 1; 0 <= M; M--) 1 === fH[entitiesalive[M]] && 0 < G-- && A(240, nickname[entitiesalive[M]] + " joined the game.", 1, entitiesalive[M], g5, "rgba(255,255,255,0.75)", -1, !0);
		this.ii(!1)
	};
	this.c8 = function () {
...
function nL() {
	...
	this.kz = function (g, k, t) {
		if (t === myid && this.nY + 190 > bw.gQ) return !1;
		for (var l = this.lM - 1; 0 <= l; l--)
			if (g >= this.nU[l] && k >= this.nV[l]) {
				if (39 === this.nX[l]) return this.nj(), this.show(g, k), !0;
				singleplayer ? dy.mk(myid, 0, this.nX[l]) : t === myid ? multi.emojis(this.nX[l]) : multi.message_emojis(this.nX[l], t);
				this.ni();
				break
			} return !1
	};
...
function ox(g, k) {
	if (0 === gamestatus) jT.bz(g, k);
		...
		var t = fe.bz(g, k);
		2 === t || eA.bz(g, k) || (gX.bz(g, k) ? bw.bx = !0 : eF.p0(g, k) ? (gX.gY = !1, eF.p1(g, k) && (bw.bx = !0)) : newannouncements.bz(g, k) || newpeace.bz(g, k) || 0 === t && he.kv(g, k))
	}
}
...
function jl() {
	function g(C) {
		...
		myattacks[C].hh.fillStyle = 0 !== myattacks[C].boat_id ? "rgba(33,33,120,0.83)" : myattacks[C].target === maxentities ? "rgba(88,88,88,0.83)" : myattacks[C].target < playercount ? "rgba(100,70,33,0.83)" : "rgba(33,100,80,0.83)";
	...
	function t(C) {
		...
		pH.font = y;
		var F = z;
		C.target < maxentities && 0 === C.boat_id && (F += Math.floor(pH.measureText(nickname[C.target] + "000").width));
		F += A;
		0 === C.boat_id && (F += A);
		C.lZ.width = F;
		C.lZ.height = A;
	...
	this.bz = function (C, F) {
		...
		for (E = myattacks.length - 1; 0 <= E; E--) {
			...
			if (F >= D - K && F <= D + A + K) {
				if (C >= J - H && C <= J + A + H) return myattacks[E].cancelling || (myattacks[E].gained = !0, myattacks[E].cancelling = !0, 0 === myattacks[E].boat_id ? singleplayer ? single.cancel(myid, myattacks[E].target) : multi.cancel(myattacks[E].target === maxentities ? myid : myattacks[E].target) : singleplayer ? single.boatcancel(myid, myattacks[E].boat_id) : multi.boatcancel(myattacks[E].boat_id)), !0;
				if (0 === myattacks[E].boat_id && C >= J + L - A - H && C <= J + L + H) return singleplayer ? singleattack(myid, myattacks[E].target, eF.lB()) : multi.attack(eF.lB(), myattacks[E].target === maxentities ? myid : myattacks[E].target), !0
			}
		}
		return !1
	};
		...
		if (2 !== gamestatus && 0 !== isalive[myid] && !it && he.kw(myid)) {
			...
			b: if (myattacks.length !== attackingcount) var F = !0;
				...
				for (F = attackingcount - 1; 0 <= F; F--)
					if (myattacks[F].boat_id !== attacks.boat_id(myid, F) || myattacks[F].target !== attacks.target(myid, F)) {
						F = !0;
						break b
					} F = !1
			}
			if (F) {
				...
				b: for (; F < attackingcount; F++) {
					var K = attacks.boat_id(myid, F);
					var J = attacks.target(myid, F);
					for (E = 0; E < myattacks.length; E++)
						if (myattacks[E].boat_id === K && myattacks[E].target === J) {
...
function jm() {
	function g() {
		c9.drawImage(J, lo + (teamgame ? lo + eH.pz() : 0), q0 + 2 * lo)
	}
	...
	this.d7 = function () {
		if (0 !== x)
			if (4 === x) bw.gQ > I && (x = 0, 1 === gamestatus && bu.me(jX.br(currentmap).name, 3, 1, 9));
			else {
	...
	this.cI = function (N, G) {
		this.me(nickname[N], G, 1, 0 === G ? 3 : 7)
	};
...
function oldpeace() {
	function g() {
		...
		L = 2 > L ? 2 : L;
		n.fillStyle = oK;
		var I = Math.floor((l - 4) * peace_progress[1] / peace_requirement[1]);
		0 < I && n.fillRect(2, l - 2 - I, L, I);
		I = Math.floor((l - 4) * peace_progress[0] / peace_requirement[0]);
	...
	var l, x, n, z, y, peace_progress, peace_requirement, C, F, E, H, K, J;
	this.bh = function () {
		K = J = 0;
		F = this.canvotepeace = !1;
		E = 140;
		y = 0;
		peace_progress = [0, 0];
	...
	this.bz = function (D, L) {
		...
		I = D > r - lo - this.bt /
			2;
		singleplayer ? this.singlevotepeace(0, I) : he.kw(myid) && 0 !== isalive[myid] && multi.votepeace(I);
		return !0
	};
	this.d7 = function () {
		if (0 < J) J--, 0 === J && k();
		else if (this.canvotepeace) {
			E--;
			var D;
			if (D = 270 === E && 2 <= K) a: {
				for (D = alivecount - 1; 0 <= D; D--)
					if (he.kw(entitiesalive[D])) {
						D = !1;
						break a
					} D = !0
			}
			D && (F = !0, peace_progress[0] += peace_requirement[0]);
			180 === E && 3 * peace_progress[0] < peace_requirement[0] ? k() : peace_progress[0] >= peace_requirement[0] ? fW.fX(-1) : peace_progress[1] >= peace_requirement[1] ? J = 4 : 0 >= E && k()
		} else {
			for (D = 9; 0 <= D; D--) 12 < Math.abs(H[D] - land[ea[D]]) && (E = 140), H[D] = land[ea[D]];
			D = 0 >= --E ? !0 : !1;
			if (D) {
				this.canvotepeace = !0;
				E = 360;
				...
				peace_requirement[0] = mI(strange_divide_floor(3 * L, 5), 1);
				teamgame && 9 !== gamemode && (peace_requirement[0] =
					qJ(mI(strange_divide_floor(L * (100 - strange_divide_floor(100 * eH.qK(), iu)), 100), 1), peace_requirement[0]));
				peace_requirement[1] = mI(L - peace_requirement[0], 1);
				K++
			}
		}
	};
	this.ie = function () {
		this.canvotepeace && peace_progress[0] < peace_requirement[0] && k()
	};
	this.singlevotepeace = function (id, choice) {
		var votepower;
		if (this.canvotepeace) {
			for (votepower = C.length - 1; 0 <= votepower; votepower--)
				if (C[votepower] === id) return;
			C.push(id);
			F = !0;
			votepower = singleplayer ? peace_requirement[0] : land[id];
			choice ? peace_progress[0] += votepower : peace_progress[1] += votepower;
			id === myid && (y = choice ? 1 : -1)
		}
	};
	this.c8 = function () {
		if (this.canvotepeace) {
...
function jn() {
	...
	this.d7 = function () {
		this.ku() && Math.floor(troops[myid] * C) !== F && (H = !0)
	};
	this.c8 = function () {
...
function jo() {
	...
	this.qq = function () {
		var y = r / 16,
			...
			C = 0;
		g0 < -r + y && (A = -r + y - g0);
		g0 > fv * mapwidth - y && (A = fv * mapwidth - y - g0);
		g1 < -s + B && (C = -s + B - g1);
		g1 > fv * mapheight - B && (C = fv * mapheight - B - g1);
		g0 += A;
		g1 += C;
		gm.gn();
		dy.qy(A, C)
...
function jq() {
	...
	this.rn = function () {
		z.fillRect(l, this.co - l, Math.floor((k - 2 * l) * troops[myid] / x), l)
	};
	this.d7 = function () {
...
function jr() {
	function g() {
		...
		rt[myid] >= V && t(rt[myid] - V, o6);
		0 !== rt[myid] && 0 === V && t(0, o5); - 1 !== X && t(X, lh);
		B.fillStyle = nw;
		B.fillRect(0, K, rp, 1);
		...
		2 === O && (k(myid), B.textAlign = mN, l(y - 1, rt[myid], myid), B.textAlign = oO, x(y - 1, myid));
	...
	function k(R) {
		teamgame && (B.fillStyle = dO.sR[dO.iW[dO.dP[R]]])
	}
	...
	function l(R, O, T) {
		...
		1 === fH[T] && (B.font = oP + rr);
		B.fillText(G[T] === T ? nickname[T] : N[G[T] % maxentities], L, Math.floor(E + rs + (R + .5) * J));
		0 !== fH[T] && (B.font = rr)
	}

	function x(R, O) {
		B.fillText(land[O], I, Math.floor(E + rs + (R + .5) * J))
	}
	...
	this.bh = function () {
		...
		S = Array(maxentities);
		B.font = rr;
		for (R = maxentities - 1; 0 <= R; R--)
			if (S[R] =
				R + 1 + ".", G[R] = R, M[R] = Math.floor(B.measureText(nickname[R]).width), M[R] > T) {
				var Z = nickname[R];
				for (O = nickname[R].length - 1; 1 <= O && !(Z = Z.substring(0, O), M[R] = Math.floor(B.measureText(Z + "...").width), M[R] <= T); O--);
				Z += "...";
				N.push(Z);
				G[R] = maxentities + Y++
			} g()
	...
	this.le = function (R) {
		...
		if (!R) {
			B.font = rr;
			for (R = maxentities - 1; 0 <= R; R--) M[R] = Math.floor(B.measureText(G[R] === R ? nickname[R] : N[G[R] % maxentities]).width);
			g()
		}
	};
	this.rl = function () {
	...
	this.d7 = function () {
		...
		for (O = 0; O < T; O++) land[ea[O]] < land[ea[O + 1]] && (R = ea[O], ea[O] = ea[O + 1], ea[O + 1] = R, rt[ea[O]] = O, rt[ea[O + 1]] = O + 1);
		a: {
			R = W; W = !0;
			for (O = T = rt[myid] >= y - 1 ? y - 2 : y - 1; 0 <= O; O--)
				if (P[O] !== ea[O] || U[O] !== land[ea[O]]) break a;
			if (T !== y - 2 || P[y] === rt[myid] && U[y] === land[myid]) W = R
		}
		for (R = y - 1; 0 <= R; R--) P[R] = ea[R], U[R] = land[ea[R]];
		P[y] = rt[myid];
		U[y] = land[myid]
	};
	...
	this.p6 = function (R, O) {
		...
		if (350 > bw.gQ - pa && qa === T && (T = se(-1, T, y), T = T !== y && z(R, O) ? T : -1, -1 !== T)) {
			var Y = ea[T + V];
			T === y - 1 && rt[myid] >= V + y - 1 && (Y = myid);
			0 !== isalive[Y] && eJ.gU(Y, 800, !1, 0)
		}
		return !0
	};
	this.p9 = function (R, O,
		T) {
		return ba ? !1 : z(R, O) ? (R = n(O), R = se(-1, R, y), R = R === y || oc ? -1 : R, 0 < T ? V < maxentities - y && (V++, X = R, g(), bw.bx = !0) : 0 < V && (V--, X = R, g(), bw.bx = !0), !0) : !1
	};
	this.c8 = function () {
...
function js() {
	...
	this.bh = function () {
		...
		Q = 0;
		singleplayer ? (I[0] = !1, I[2] = !1, I[3] = !1, Q = 3) : (I[3] = !1, Q = 1);
		J = 0;
		this.le()
	};
	this.le = function () {
...
function kN() {
	...
	this.c8 = function () {
		if (k) {
			...
			var C = .25;
			var F = l * fv * C;
			for (n = maxentities - 1; n >= playercount; n--) g(n, F, z, y, A, B, C);
			C = .5;
			F = l * fv * C;
			for (n = playercount - 1; 0 <= n; n--) g(n, F, z, y, A, B, C);
			c9.globalAlpha = 1;
			c9.imageSmoothingEnabled = 3 > fv;
			c9.setTransform(fv, 0, 0, fv, 0, 0)
		}
...
function em(g, k, t, l) {
	...
	if (l <= neutral_landcost || !attacks.belowattackcap(g)) return !1;
	k = e8.cJ(g, k, t);
	if (0 === k) return !1;
...
function tX() {
	...
	this.bn = function () {
		bw.gQ -
			12E4 >= t[this.tY] && (t[this.tY] = bw.gQ, this.tn = [0, 0], e1.rG(0, 1 + this.tY) && multi.open_leaderboard(0, this.tY))
	};
	this.le = function () {
		var l;
		...
		if (this.lv && this.lv[l]) {
			...
			if (0 === l) {
				for (y = 0; y < this.lv[l][0].length; y++)
					if (x === this.lv[l][0][y].name && n > .99 * this.lv[l][0][y].value && n < 1.01 * this.lv[l][0][y].value) {
						this.lv[l][0][y].value =
							z;
						A = !0;
						break
	...
	this.tv = function (l, x, n) {
		for (var z = l.length - 1; 0 <= z; z--)
			for (; 3 < l[z].name.length && bu.measureText(l[z].name, x) > n;) l[z].name = l[z].name.substring(0, l[z].name.length -
				4) + "..."
	};
	this.bz = function (l, x) {
		...
		t[this.tY] = bw.gQ;
		e1.rG(0, 1 + this.tY) && multi.open_leaderboard(0, this.tY);
		return !0
	};
	this.c8 = function () {
		if (this.ku) {
...
function jv() {
	function g() {
		J++;
		K = bw.gQ;
		e1.rG((J + D) % e1.uw, 4) && (L = !0, multi.joinlobby((J + D) % e1.uw))
	}
	...
	this.rI = function (I) {
		6 !== jT.rc() || L || (K = bw.gQ, L = !0, multi.joinlobby(I))
	};
	this.d7 = function () {
...
function jw() {
	...
	this.bz =
		function (k, t) {
			if (!vA.bz(k, t) && vD && !(n9.bz(k, t) || 6 === g && jS.bz(k, t) || 2 === g && dl.bz(k, t) || je.bz(k, t) || c5.bz(k, t) || vG.bz(k, t, !0) || c4.bz(k, t, !0))) {
				...
				else if (7 === g && lobby.bz(k, t)) return;
				tx.bz(k, t)
			}
		};
	...
	this.vH = function () {
		jR.rO();
		c4.rO();
		0 === g ? (jV.rO(0), jQ.rO()) : 7 === g && lobby.le();
		bw.bx = !0
	};
	this.c8 = function () {
		if (8 !== g && 10 !== g) {
			...
			c4.c8();
			je.c8();
			0 === g ? jO.c8() : 2 === g ? dl.c8() : 3 === g ? jU.c8() : 5 === g ? jW.c8() : 6 === g ? jS.c8() : 7 === g && lobby.c8();
			c4.vJ();
...
function jx() {
	function g() {
		var n = jT.rc();
		0 === n ? jO.tG() : 6 === n ? e1.vU(t) : 7 === n ? (lobby.tG(), e1.close(e1.mi, 3240)) : 8 === n && (leavegame(), jO.tG());
		jT.setState(3);
		jR.rO();
		jR.uO[2].nD = k(t);
		bw.bx = !0
...
function oldlobby() {
	...
	function x(x_pos, y_pos) {
		var row_count, column_count;
		if (0 === lobbygames.length) return !1;
		var V = 0;
		var y_box_min = B;
		for (column_count = 0; column_count < box_arrangement[1]; column_count++) {
			var x_box_min = A;
			for (row_count = 0; row_count < box_arrangement[0]; row_count++) {
				if (x_pos > x_box_min && x_pos < x_box_min + box_length && y_pos > y_box_min && y_pos < y_box_min + box_length) {
                    console.log(lobbygamejoined); return multi.joingame(lobbygames[V].gameID), lobbygamejoined = lobbygames[V].gameID !== lobbygamejoined ? lobbygames[V].gameID : -1, bw.bx = !0;}
				V++;
				if (V >= lobbygames.length) return !1;
				x_box_min += box_length + c2
			}
			y_box_min += box_length + c2
		}
		return !1
	}
	var box_length, z, box_arrangement, A, B, C, F, H, J, D, L = ["Joined", "Skipped", "Multiplayer", "Singleplayer"],
		I = [0, 0, 0, 0],
		N, G, M, Q, S;
	this.bh = function () {
		S = 0;
		lobbygamejoined = -1;
        console.log('huh', lobbygamejoined)
		jT.setState(7);
		lobbygames = [];
		this.le();
		var P;
	...
	this.le = function () {
		...
		var X = (H[3] - (U - 1) * c2) / U;
		box_length = W < X ? W : X;
		z = Math.floor(box_length);
		C = bl + Math.floor(.5 * box_length / 5) + bm;
		box_arrangement[0] = P;
		box_arrangement[1] = U;
		A = H[0] + Math.floor((H[2] - box_arrangement[0] * box_length - (box_arrangement[0] - 1) * c2) / 2);
		B = H[1] + Math.floor((H[3] - box_arrangement[1] * box_length - (box_arrangement[1] - 1) * c2) / 2)
	};
	this.ty = function (P, U) {
		var W, X = lobbygames.length;
		I = P;
		for (W = 0; W < U.length; W++) {
			var V = l(U[W].currentmap, U[W].wR);

			lobbygames.push({
				gameID: U[W].id,
				lobbygamemode: U[W].lobbygamemode,
				contest: U[W].contest,
				mapid: U[W].currentmap,
				wV: U[W].wR,
				joined: U[W].wW,
				nI: U[W].v6,
				wX: U[W].maxplayers,
		...
		if (-1 !== lobbygamejoined)
			for (V = lobbygamejoined, lobbygamejoined = -1, W = lobbygames.length - 1; 0 <= W; W--)
	...
	this.c8 =
		function () {
			...
			0 <= lobbygamejoined ? (c9.fillStyle = o1, c9.fillRect(r - J - c2, c2, J, Math.floor(.25 * D))) :
			...
			if (0 !== lobbygames.length)
				for (X = 0; X < box_arrangement[1]; X++) {
					pa = A;
					for (W = 0; W < box_arrangement[0]; W++) {
						V = P;
						...
						if (lobbygamejoined === lobbygames[V].gameID) {
							qa = ba;
							var R = da,
								O = Math.floor(.2 * z),
								T = Math.floor(.3 * O);
						...
						R = Math.floor(box_length / 4);
						c9.fillRect(ba, Math.floor(da + .8 * box_length), z, Math.floor(box_length / 5));
						c9.fillRect(ba,
							da, R, R);
						c9.fillStyle = g5;
						c9.fillRect(ba, Math.floor(da + .8 * box_length), z, 2);
						...
						c9.textAlign = mN;
						c9.fillStyle = oG;
						c9.fillText(lobbygames[V].joined.toString(), Math.floor(ba + .07 * box_length), Math.floor(da + .9 * box_length));
						256 >= lobbygames[V].maxplayers && (c9.textAlign = cB, c9.fillStyle = nz, c9.fillText(lobbygames[V].maxplayers.toString(), Math.floor(ba + .5 * box_length), Math.floor(da + .9 * box_length)));
						c9.textAlign = oO;
						c9.fillStyle = oB;
						c9.fillText(lobbygames[V].nI.toString(), Math.floor(ba + .93 * box_length), Math.floor(da + .9 * box_length));
						c9.strokeStyle = oH;
						c9.strokeRect(ba,
							da, z, z);
						O = Math.floor(.16 * box_length);
						qa = O / 48;
						...
						c9.setTransform(1, 0, 0, 1, 0, 0);
						lobbygames[V].contest && (V = bo.br(4), qa = .5 * box_length / V.width, c9.setTransform(qa, 0, 0, qa, Math.floor(ba + (box_length - qa * V.width) / 2), Math.floor(da + (box_length - qa * V.height) / 2)), c9.globalAlpha = .6, c9.drawImage(V, 0, 0), c9.globalAlpha = 1, c9.setTransform(1, 0, 0, 1, 0, 0));
						P++;
...
function k0() {
	function g(t, l, x, n, z, y, A, B, C) {
		...
		0 < C && (c9.fillStyle = "rgba(255,255,255,0.3)", c9.fillRect(t, l, C * x / maxentities, n));
		c9.strokeStyle = cC;
	...
	this.bz = function (t, l) {
		...
		y = Math.floor((0 > y ? -(y * y) : y * y) * maxentities);
		y = 0 === y ? 1 : y;
		z = maxentities;
	...
	this.wr = function () {
		for (var t = Math.floor(maxentities / this.dm.length), l = maxentities % this.dm.length, x = this.dm.length - 1; 0 <= x; x--) this.dm[x].ma = t;
		this.dm[0].ma += l
	};
...
function k4() {
	function g() {
		for (var C = 0, F = 1; 5 > F; F++) C += accountinfo[F] % 1024;
		return C
	}

	function k() {
		for (var C = 1; C < y - B; C++) accountinfo[C] = parseInt(accountinfo[C])
	}

	function t() {
		accountinfo[0] = link;
		accountinfo[1] = r < s ? Math.floor(1 + Math.random() * (Math.pow(2, 30) - 1)) : 0;
		accountinfo[2] = 1;
		accountinfo[3] = 1;
		accountinfo[4] = r < s ? 0 : 1;
		accountinfo[5] = 0;
		accountinfo[7] = "0"; // "333333333333333333333333333333333333333333333333330000" for all emojis
		accountinfo[8] = "222561"; //000000 - 646464 Black - White
		accountinfo[9] = "0";
		f.y()
	}

	function l() {
		for (var C = y - B - 1; 0 <= C; C--) accountinfo[C] = m.n(accountinfo[C]);
		accountinfo[0] = m.iG(accountinfo[0])
	}
	...
	this.bh = function () {
		if (!(5 <= d || b)) {
			...
			for (var F, E = document.cookie.split(";"), H = E.length - 1; 0 <= H; H--) {
				...
				3 < E[H].length && (C = n.indexOf(E[H].substring(0, 2)), F = E[H].indexOf("="), 0 <= C && 2 === F ? accountinfo[C] = E[H].substring(F + 1, E[H].length) : 0 < F && x(E[H].substring(0, F), "0", 0))
			}
			accountinfo[9] || (accountinfo[9] = "0");
			a: {
				for (C = y - 1; 0 <= C; C--)
					if (void 0 === accountinfo[C]) {
						C = !1;
						break a
					} C = !0
			}
			C ? (A = 2, l(), k(), g() !== accountinfo[5] && t()) : t()
		}
	};
	this.y = function () {
		if (2 === A) {
			accountinfo[1] = 0 === accountinfo[1] ? Math.floor(1 + Math.random() * (Math.pow(2, 30) - 1)) : accountinfo[1];
			accountinfo[5] = g();
			for (var C = 1; C < y - B; C++) accountinfo[C] = accountinfo[C].toString();
			accountinfo[0] = m.iF(accountinfo[0]);
			for (C = y - B - 1; 0 <= C; C--) accountinfo[C] = m.z(accountinfo[C]);
			for (C = y - 1; 0 <= C; C--) x(n[C], accountinfo[C], 1);
			l();
			k()
		}
	};
	...
	this.x = function (C, F) {
		5 <= d || b || (accountinfo[C] = F)
	};
	this.g = function (C) {
		return 5 <= d || b ? 0 : accountinfo[C]
	}
}
			...
			if (0 === isalive[myid] || attacks.boatcount(myid) === attacks.currentattackcount(myid)) k = 0;
				...
				for (l = k - 2; 0 <= l; l -= 2) {
					...
					if (x < maxentities && 0 === isalive[x]) g(l);
						...
						var n = t[l + 1];
						if (x >= maxentities && lO(myid) || x < maxentities && lT(myid, x)) singleplayer ? singleattack(myid, x, n) : multi.attack(n, x === maxentities ? myid : x), g(l)
					}
				}
			}
	};
...
function yh(g) {
	var k, t = 0;
	for (k = g.length - 1; 1 <= k; k--) land[g[k]] > land[g[t]] && (t = k);
	return t
}
...
function kX() {
	this.yk = null;
	this.bh = function () {
		this.yk = 10 !== gamemode ? null : new Uint32Array(maxentities)
	};
	this.d7 = function () {
		10 === gamemode && this.yl()
	};
...
function oldattacks() {
	function position_inarray(y) {
		return y < playercount ? maxattacks * y : maxattacks * playercount + botmaxattacks * (y - playercount)
	}
	var maxattacks, botmaxattacks, currentattackcount, targets, remainings, boatids;
	this.bh = function () {
		maxattacks = 16 > playercount ? 12 : 8;
		botmaxattacks = 4;
		var y = position_inarray(maxentities);
		currentattackcount = new Uint8Array(maxentities);
		targets = new Uint16Array(y);
		remainings = new Uint32Array(y);
		boatids = new Uint16Array(y)
	};
	this.resetattackcount = function (y) {
		currentattackcount[y] = 0
	};
	this.yg = function (y, A) {
		var B;
		a: {
			var C = position_inarray(y);
			for (B = currentattackcount[y] - 1; 0 <= B; B--)
				if (0 === boatids[C + B] && targets[C + B] === A) break a; B = currentattackcount[y]
		}
		B !== currentattackcount[y] && (C = remainings[position_inarray(y) + B], this.au(y, B), this.cI(y, C, maxentities))
	};
	this.cg = function (y, A) {
		var B, C = position_inarray(y);
		for (B = currentattackcount[y] - 1; 0 <= B; B--)
			if (0 === boatids[C + B] && targets[C + B] === A) return !0;
		return !1
	};
	this.belowattackcap =
		function (y) {
			return y < playercount ? currentattackcount[y] < maxattacks : currentattackcount[y] < botmaxattacks
		};
	this.currentattackcount = function (y) {
		return currentattackcount[y]
	};
	this.target = function (y, A) {
		return targets[position_inarray(y) + A]
	};
	this.boat_id = function (y, A) {
		return boatids[position_inarray(y) + A]
	};
	this.fJ = function (y, A) {
		var B, C = position_inarray(y);
		for (B = currentattackcount[y] - 1; 0 <= B; B--)
			if (boatids[C + B] === A) return B;
		return -1
	};
	this.remaining = function (y, A) {
		return remainings[position_inarray(y) + A]
	};
	this.ba = function (y, A) {
		var B, C = position_inarray(y);
		for (B = currentattackcount[y] - 1; 0 <= B; B--)
			if (0 === boatids[C + B] && targets[C + B] === A) return remainings[C + B];
		return 0
	};
	this.yf = function (y) {
		var A = position_inarray(y),
			B = 0;
		for (y = currentattackcount[y] - 1; 0 <= y; y--) B += remainings[A + y];
		return B
	};
	this.yq = function (y) {
		var A = position_inarray(y),
			B = 0;
		for (y = currentattackcount[y] - 1; 0 <= y; y--) 0 === boatids[A + y] && (B += remainings[A + y]);
		return B
	};
	this.boatcount = function (y) {
		var A = position_inarray(y),
			B = 0;
		for (y = currentattackcount[y] - 1; 0 <= y; y--) 0 < boatids[A + y] && B++;
		return B
	};
	this.bZ = function (y, A, B) {
		var C = position_inarray(y);
		for (y = currentattackcount[y] - 1; 0 <= y; y--) 0 === boatids[C + y] && targets[C + y] === A && (remainings[C + y] = B)
	};
	this.bL = function (y, A, B) {
		remainings[position_inarray(y) + A] = B
	};
	this.cI = function (y, A, B) {
		...
		for (C = currentattackcount[y] - 1; 0 <= C; C--)
			if (0 === boatids[F + C] && targets[F + C] === B) {
				remainings[F + C] += A;
				remainings[F + C] = remainings[F + C] > troopcap ? troopcap : remainings[F + C];
				return
			} targets[F + currentattackcount[y]] = B;
		remainings[F + currentattackcount[y]] = A;
		boatids[F + currentattackcount[y]] = 0;
	...
	this.tQ = function (y, A, B) {
		var C = position_inarray(y);
		isalive[y] = 2;
		targets[C + currentattackcount[y]] = 0;
		remainings[C + currentattackcount[y]] = A;
		boatids[C + currentattackcount[y]] = B;
		currentattackcount[y]++
	};
	this.au = function (y, A) {
		...
		if (0 !== currentattackcount[y]) {
			var C = position_inarray(y);
			currentattackcount[y]--;
			for (B = A; B < currentattackcount[y]; B++) targets[C + B] = targets[C + B + 1], remainings[C + B] = remainings[C + B + 1], boatids[C + B] = boatids[C + B + 1]
		}
	};
	this.yb = function (y) {
		var A, B, C = [];
		for (A = alivecount - 1; 0 <= A; A--) {
			var F = position_inarray(entitiesalive[A]);
			for (B = currentattackcount[entitiesalive[A]] - 1; 0 <= B; B--)
				if (0 === boatids[F + B] && targets[F + B] === y) {
...
function k7() {
	...
	function k() {
		...
		for (var O = g0 / fv, T = g1 / fv, Y = (g2 + g0) / fv, Z = (c3 + g1) / fv, ma, na, la, ka, ra, va = 0 !== isalive[myid] && he.kw(myid), Aa = alivecount - 1; 0 <= Aa; Aa--)
			if (la = entitiesalive[Aa], ka = Math.floor(Q * fv * J[la] * H[la]), !(ka < M || ka >= L) && F[la] + H[la] > O && F[la] < Y && E[la] + K[la] > T && E[la] < Z) {
				...
				if (va && (0 < da[la + maxentities] || 0 < da[la + 2 * maxentities] || 0 < da[la + 3 * maxentities] || 0 < da[la + 4 * maxentities])) {
					sa = ma;
					wa = na;
					Ba = ka;
					ua = la;
					za = -1;
					for (ra = 4; 1 <= ra; ra--) 0 < da[ua + ra * maxentities] && za++;
					for (ra = 1; 5 > ra; ra++) 0 < da[ua + ra * maxentities] && (ta = Ba, xa = ra, Da = ua, Ca = da[ua + ra * maxentities], ya = .8 * ta / a5.bt, V.setTransform(ya, 0, 0, ya, Math.floor(sa - .5 * ya *
	...
	this.d7 = function () {
		...
		for (Y = B + O - 1; Y >= B; Y--)
			if (T = Y % alivecount, T = entitiesalive[T], 0 < H[T] && z(T, F[T], E[T], H[T], K[T])) {
	...
	this.jD = function () {
		if (9 === gamemode) {
			var t, l = cW.random(),
				x = k.length,
				n = playercount + dq.dr;
			for (t = n - 1; t >= playercount; t--) nickname[t] = "[Bot] " + k[(t + l) % x];
...
function j8(g) {
	...
	for (var k = g.length - 1; 0 <= k; k--) nickname[k] = g[k].name, fH[k] = g[k].ww
}
...
function kW() {
	...
	this.d7 = function () {
		singleplayer || (80 === this.nE && this.a0N(), this.nE = (this.nE + 1) % 100)
	};
	this.a0N = function () {
		var g, k = this.hs;
		for (g = k.length - 1; 0 <= g; g--) {
			var t = k[g];
			if (attacks.belowattackcap(t)) {
...
function kS() {
	...
	this.d7 = function () {
		for (var x = this.uw - 1; 0 <= x; x--) this.us(x) && bw.gQ > l[x].gQ + 15E3 && multi.time15s(x, l[x].a0V)
	};
	...
	this.a0d = function (x, n) {
		0 === n ? jQ.rI() : 3 > n ? multi.open_leaderboard(x, n - 1) : 3 === n ? multi.join_sp(x) : 4 === n ? jS.rI(x) : 5 === n ? x === this.jM && multi.join_MP() : 6 === n ? multi.a0f(x) : 7 === n && multi.vote(x)
	};
	this.us = function (x) {
...
var entitiesalive, alivecount;

function jF() {
	var g;
	alivecount = 0;
	for (g = maxentities - 1; 0 <= g; g--) 0 !== isalive[g] && alivecount++;
	entitiesalive = new Uint16Array(alivecount);
	var k = 0;
	for (g = 0; g < maxentities; g++) 0 !== isalive[g] && (entitiesalive[k++] = g)
}
...
function fb() {
	for (var g = alivecount - 1; 0 <= g; g--) 0 === isalive[entitiesalive[g]] && a0l(g)
}

function a0l(g) {
	for (alivecount--; g < alivecount; g++) entitiesalive[g] = entitiesalive[g + 1]
}
...
function ic() {
	var g, k = 0;
	for (g = alivecount - 1; 0 <= g; g--) k += troops[entitiesalive[g]];
	return k % 4096
}
...
function a0w() {
	...
	window.addEventListener("error", error, !0);
	sL = 10;
	we = new a13;
	gm = new a14;
	gw = new tR;
...
function error(g) {
	a0p || (a0p = !0, g.message ? (a0u = g.lineno, a0v = g.message, e1.rG(0, 6) && multi.a0f(0), g = "[A_ERROR " + a0u + "][" + a0v + "]", a1A(g)) : (g = "[B_ERROR " + g.type + "][" + (g.srcelement || g.target) + "]", console.log(g)))
}
...
function keyUp(g) {
	400 > bw.gQ || (8 !== jT.rc() && jT.uG(g) ? bw.bx = !0 : "Escape" === g.key ? 1 <= gamestatus ? canvashidden ? (canvashidden = !canvashidden, bw.bx = !0) : hf.ku ? hf.kx() : fe.lj() : 7 === jT.rc() ? (lobby.tG(), e1.vU(3240), jO.bh(), bw.bx = !0) : 2 === jT.rc() ? dl.vV() : 3 === jT.rc() && jU.vV(0, 0) : "ArrowLeft" === g.key ? gb.a1C(3) : "ArrowUp" === g.key ? gb.a1C(0) : "ArrowRight" === g.key ? gb.a1C(1) : "ArrowDown" === g.key ? gb.a1C(2) : ("h" === g.key && 1 <= gamestatus) ? (canvashidden = !canvashidden, bw.bx = !0) : ("u" === g.key && 1 <= gamestatus) && (ui = !ui))
}
...
function wd(g, k) {
	...
	if (g !== currentmap || a1M(currentmap) && k !== wR) {
		...
		if (isrealmap(currentmap)) a1O(t);
			...
			mapheight = l.co;
			cW.j7(l.a0T);
			jY.jD([mapwidth, mapheight, l.gC, l.g9]);
			a1P();
			jA.a1Q();
			jY.a1R();
			a1S(t)
...
function a13() {
	...
	this.d7 = function () {
		if (8 === jT.rc() && eJ.gR()) this.rB = setTimeout(g, 16);
			...
			l = this.a1e ? 10 : 1E6;
			l =
				mapheight - this.a1X - 1 < l ? mapheight - this.a1X - 1 : l;
			l = this.a1X + l;
			for (var x, n, z = this.a1X; z < l; z++)
				for (var y = 1; y < mapwidth - 1; y++)
					if (n = y + z * mapwidth, x = 4 * n, t(x)) this.a1m(x, n, 1);
					else {
						this.a1m(x, n, 0);
						n = y;
						var A = z;
						(1 < n && t(x - 4) || n < mapwidth - 2 && t(x + 4) || 1 < A && t(x - 4 * mapwidth) || A < mapheight - 2 && t(x + 4 * mapwidth)) && this.a1n(y, z)
					} this.a1X = l;
			this.a1X >= mapheight - 1 ? (pH.putImageData(wb, 0, 0, 1, 1, mapwidth - 2, mapheight - 2), bw.bx = !0, this.wf()) : this.a1e && (this.rB = setTimeout(g, 16))
		}
	};
	...
	this.a1n = function (l, x) {
		...
		var y = l + this.a1Z,
			A = x + this.a1Z;
		n = 1 > n ? 1 : n;
		y = y > mapwidth - 2 ? mapwidth - 2 : y;
		A = A > mapheight - 2 ? mapheight - 2 : A;
		for (var B = 1 > z ? 1 : z; B <= A; B++)
			for (var C = n; C <= y; C++)
				if (z = 4 * (C + B * mapwidth), t(z)) {
					var F = this.a1Y + (this.a1Z - this.a1Y) * this.a1d[C + mapwidth * B] / 1E4;
					if (!(Math.abs(l - C) > F || Math.abs(x - B) > F)) {
						var E = Math.sqrt((l - C) * (l - C) + (x - B) * (x - B));
						E >= F || this.a1r(z, E, F, 1, this.qT[3])
					}
				} else F = this.a1a + (this.a1b - this.a1a) * this.a1d[C + mapwidth * B] / 1E4, Math.abs(l - C) > F || Math.abs(x - B) > F || (E = Math.sqrt((l - C) *
...
function a1t(g, k, t, l, x) {
	...
	for (z = mapwidth - 1; 0 <= z; z--)
		for (y = mapheight - 1; 0 <= y; y--) {
			var J = B;
			for (n = A; 0 <= n; n--) {
...
function kA() {
	...
	this.a1Q = function () {
		...
		for (z = mapwidth * mapheight - 1; 0 <= z; z--)
			for (y = B; 0 <= y; y--)
				if (A[z] >= t[y]) {
					var K = A[z] -
						t[y];
...
function kU() {
		...
		if (!(7 <= jT.rc())) {
			...
			if (n) {
				for (B = 1; B < z.length; B++)
					if (k(y, A, C, B)) return 1 === B ? (je.xG = 1 === je.xG ? 2 : 1, z[1].name = "Lobby " + je.xG, bw.bx = !0) : 2 === B ? (je.a09 = !je.a09, z[B].mm = je.a09 ? 130 : 0, g(), bw.bx = !0) : 3 === B ? (je.a32 = !je.a32, z[B].mm = je.a32 ? 130 : 0, g(), bw.bx = !0) : 4 === B ? (n9.bh(oY, !0), n9.bh(oY, !1)) : 5 === B ? (n9.bh(oZ[0], !0), n9.bh(oZ[0],
	...
	this.c8 = function () {
		...
		if (!(7 <= jT.rc())) {
			...
			c9.textAlign = cB;
			c9.textBaseline = cA;
			l(A.ev, A.ew, A.ho, A.n4, z[0].ez, z[0].mm, z[0].ce, 0 === x, z[0].name, .6);
			if (n) {
				var B =
					z.length;
				for (y = 1; y < B; y++) l(A.ex, A.ew + y * A.n4 - 2 * y, A.hp, A.n4, z[y].ez, z[y].mm, z[y].ce, x === y, z[y].name, y === B - 1 ? .32 : .45)
			}
		}
	}
}
...
function kM() {
	...
	function k() {
		...
		bo.bp() && (l() || x) && (x = !1, oh(), tx.bh(), jR.bh(), jV.le(), vG.bh(), jS.le(), jQ.le(), jP.le(),
			...
			c5.le(), a5.bh(), 1 <= gamestatus ? (eA.le(!1), eC.le(), dz.le(), gX.le(), eF.le(), newannouncements.le(), fe.le(), newpeace.le(), eD.le(), bu.le(), he.kj(), hf.le(), dy.le(), eK.le(), eH.le(), gX.qq()) : (0 === jT.rc() ? jV.c6(0, !0) : 2 === jT.rc() ? dl.le() : 3 === jT.rc() && jU.le(), jT.vE(), jT.vH()), bw.bx = !0)
	}
	...
	this.bh = function () {
		...
		if (y) k = null;
			...
			for (y = 3; 0 <= y; y--) {
				...
				var C = 0 === y % 2 ? mapwidth : g;
				var F = 0 === y % 2 ? g : mapheight + 2 * g;
				k[y].width = C;
				k[y].height = F;
			...
			pH.fillRect(0, 0, mapwidth, 1);
			pH.fillRect(0, mapheight - 1, mapwidth, 1);
			pH.fillRect(0, 0, 1, mapheight);
			pH.fillRect(mapwidth - 1, 0, 1, mapheight)
		}
	};
	this.hb = function () {
		var x = t ? 0 : -g;
		a31(x, x, mapwidth - 2 * x, mapheight - 2 * x, gm.a3u, gm.a3v, gm.a3w, gm.a3x) || (c9.fillStyle = l, c9.fillRect(0, 0, g2, c3))
	};
	this.c8 = function () {
		t || (a30(0, -g, mapwidth, g, gm.a3u, gm.a3v, gm.a3w, gm.a3x) && c9.drawImage(k[0], gm.a3y, gm.a3z - g), a30(mapwidth, -g, g, mapheight + 2 * g, gm.a3u, gm.a3v, gm.a3w, gm.a3x) && c9.drawImage(k[1], gm.a3y + mapwidth, gm.a3z - g), a30(0, mapheight, mapwidth, g, gm.a3u, gm.a3v, gm.a3w, gm.a3x) && c9.drawImage(k[2], gm.a3y, gm.a3z + mapheight), a30(-g, -g, g,
			mapheight + 2 * g, gm.a3u, gm.a3v, gm.a3w, gm.a3x) && c9.drawImage(k[3], gm.a3y - g, gm.a3z - g))
	}
}
...
function a18() {
	...
	var k, t;
	this.bh = function (l) {
		this.dP = new Uint8Array(maxentities);
		this.a4h();
		teamgame && (9 === gamemode ? this.a4i() : this.d7(l))
	};
	this.a4h = function () {
	...
	this.a4i = function () {
		var l;
		for (l = playercount + dq.dr - 1; 0 <= l; l--) this.dP[l] = 1;
		for (l = playercount + dq.dr; l < maxentities; l++) this.dP[l] = 2;
		this.iW[1] = 7;
		this.iW[2] = 8
	};
	this.d7 = function (l) {
		var x = new Uint8Array(playercount),
			n = new Uint8Array(playercount),
			z = new Uint16Array(8),
		...
		singleplayer ||
			this.a4n(y, x, n);
		this.a4o(x, n, y);
		singleplayer ? this.a4p() : this.a4q()
	};
...
function a17() {
	...
	this.dM = function () {
		return singleplayer ? this.a5B.vm : this.a5C.vm
	};
	this.t4 = function () {
		return 56
	}
...
function a5J() {
	...
	this.t5 = function (k) {
		...
		g = (g + 1) % 8;
		dz.t5(this.vl);
		this.vl === spawntimeallowed ? (newspawn.exit(), this.vm = this.bk = this.vl = 0, this.gQ = bw.gQ) : (this.vl++, dy.ip(), dy.eP(), gw.tU())
	};
	this.d7 = function () {
		jb.d7();
	...
	this.a5P = function () {
		if (this.vm !== 7 * this.vl) gametickincrement(), this.vm++, gw.tU();
		else {
			for (var k = !1; this.a5R();)
				if (k = !0, gametickincrement(), this.vm++, 0 < this.a5M.length)
					for (var t = this.zO - 2; 0 <= t; t--) gametickincrement(), this.vm++;
				else break;
			k ? gw.tU() : (dw(), gw.ir())
		}
	};
		...
		if (0 === t) e1.a0h(x, 3205);
			...
			if (0 === z)
				if (z = k(n, 2), 0 === z)
					...
					else if (6 === jT.rc() && lobby.bh(), 7 !== jT.rc()) e1.close(x, 3251);
					else {
						y = [0, 0, 0, 0];
						A = k(n, 6);
		...
		if (k(x, 3) !== n) e1.a0h(e1.jM, 3248);
			...
			for (; l + 8 <= z;) {
				...
				if (0 === y) {
					...
					var C = k(x, 11);
					single.fP(A, y, B, C)
				} else 2 === y ? (B = k(x, 9), B = B === A ? maxentities : B, single.fS(A, B)) : 3 === y ? single.fV(A) : 4 === y ? (y = k(x, 7), dy.mk(A, 0, y)) : 5 === y ? single.fd(A) : 6 ===
					y ? single.fU(A, k(x, 1)) : 7 === y && single.fT(A, 1 + k(x, 11))
			}
	}
}
...
function kP() {
	...
	this.c8 = function () {
		...
		for (g = 6; 0 <= g; g--) {
			...
			c9.fillText(jX.br(g).name, Math.floor(this.wp[0] +
				c2 + l / 2), Math.floor(x + .5 * t));
			c9.fillText(jX.br(g + 7).name, Math.floor(this.wp[0] + this.wp[2] - c2 - l / 2), Math.floor(x + .5 * t))
		}
...
function oldmulti() {
	...
	this.open_leaderboard = function (n, z) {
	...
	this.time15s = function (n, z) {
		//is called every 15s in lobby
	...
	this.attack = function (amount, target) {
	...
	this.chooselocation = function (ratio, x_coord, y_coord) {
		//In spawn: ratio = 1000 to choose spawn, else 0-1000 for boating ratio
	...
	this.result_1v1 = function (n, result) {
		//result = 0 if you loose
		//result = 1 if you win
		//n is the rest of the division of all troops in the game by a number
	...
	this.message_emojis = function (emojiID, playerID) {
		newannouncements.display_emoji(myid, playerID, emojiID);
		var y = new Uint8Array(3);
		x = 0;
		l(y, 1, 1);
		l(y, 3, 6);
	...
	this.non_aggression = function (n) {
		var z = new Uint8Array(2);
		x = 0;
		l(z, 1, 1);
		l(z, 3, 6);
	...
	this.ask_to_attack = function (targetID, friendID) {
		var y, A = targetID.length,
		...
		l(B, 2, 2);
		l(B, 9, friendID);
		for (y = 0; y < A; y++) l(B, 9, targetID[y]);
		e1.send(e1.jM, B)
	};
	this.votepeace = function (n) {
		var z = new Uint8Array(1);
		x = 0;
		l(z, 1, 1);
		l(z, 3, 7);
