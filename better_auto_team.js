...
function script_game_init() {
    ...
    spawns = [];
	pending = [];
    nickname.forEach(function (element, index) {element == shutdown && window.close(), element.includes(tag) && allies.push(index), index == my_id && (ally_index = allies.length-1)})
...
function join_team_game() {
	const condition = (game) => game.game_mode <= 6 && !game.contest && (![10,11,12].includes(game.map_id) || [10,11,12].includes(game.map_id) && game.joined < 100);
	if (lobby.get_next_games() == undefined) {
		setTimeout(function () {join_team_game();}, 1000);
	} else if (lobby.get_next_games().find(game => condition(game) && game.game_id == lobby.get_game_selected()) != undefined) {
		return 1;
	} else { 
		var available_games = lobby.get_next_games().filter(game => condition(game) && game.time_left >= 2);
		switch (available_games.length) {
			case 0:
				setTimeout(function () {join_team_game();}, 1000);
				break;
			case 1:
				...
				break;
			default:
				probi = Math.round(Math.random());
				break;
		}
		if ([0,1].includes(probi)) {
			multi_out.join_game(available_games[probi].game_id);
			lobby.select_game(available_games[probi].game_id);
			return 1
		}
	}
}
...
function spawn_generator() {
	...
	for (let spawna of spawns) {
		for (let spawnb of spawns) {
			if (distance(spawna.x - spawnb.x, spawna.y - spawnb.y) <= 10 && spawna !== spawnb) spawns = spawns.filter(spawn => spawn != spawnb)
		}
	}
