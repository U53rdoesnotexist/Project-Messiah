var tick, cycle, latency, rating, opponentid, borderingpixels;
...
function tickincrement() {
	...
	borderingpixels = [];
	for (let pixelcoord of borderlandpixels[myid]) for (let side = 0; side <= 3; side++) {
		let pixelcoords = pixelcoord + offset[side];
		if (!borderingpixels.includes(pixelcoords) && !pixel.strong_isowner(myid,pixelcoords)) borderingpixels.push(pixelcoords)
	}

}

function calibrate_attack(ratio, id) { //myid then dun care amount -> 512
	id == 512 && (id = myid)
	var amount = ratio * troops[myid] / 1000
	if (id >= playercount && tick + latency <= 90 && alivecount >= 20 && ui == 1) {
		if (density(id) < 1 || bot_timing[id - playercount] <= 15) {
			console.log(amount, maxlandgain(id) * Math.min(2.4 + density(myid), 2.8))
			amount = Math.min(amount, maxlandgain(id) * Math.min(2.4 + density(myid), 2.8))
		}
	}
	ratio = amount * 1000 / troops[myid];
	if (singleplayer) singleattack(myid, id == myid ? 512 : id, ratio)
	else multi.attack(ratio, id)
}

function maxlandgain(id) {
	return Math.min(speed(myid) * (100-tick-latency) * borderingpixels.filter(pixelcoord => borderlandpixels[id].includes(pixelcoord)).length, land[id])
}

function speed(id) {
	return land[id] < 1E3 ? 1 / 4 : land[id] < 1E4 ? 1 / 3 : land[id] < 6E4 ? 1 / 2 : land[id] < 16E4 ? 1 : land[id] < 3E5 ? 2 : 3
}
...
function jh() {
	...
	this.bz = function (N, G) {
		...
		if (4 === M) return x[0] ? in_spawn ? (this.kx(), singleplayer ? (newspawn.setspawn(0, pixel.pixeltox(H), pixel.pixeltoy(H)), newspawn.exit()) : multi.chooselocation(1E3, pixel.pixeltox(H), pixel.pixeltoy(H))) : (this.kx(), newannouncements.lA(), (singleplayer || !freespawn || 300 < dz.lD()) && calibrate_attack(eF.lB(),E)) : x[8] ? (this.kx(), e5.lF(E, eF.lB())) : this.kx(), 1;
